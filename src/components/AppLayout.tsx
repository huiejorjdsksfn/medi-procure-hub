...
interface NavGroup { label: string; items: NavItem[]; roles: ProcurementRole[]; }
...
const AppLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const renderSidebar = () => (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 overflow-y-auto px-2 pb-4 space-y-1 pt-3">
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link key={item.path} to={item.path} onClick={() => setMobileOpen(false)}
                      className={`flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-all ${isActive ? "bg-primary/10 text-primary font-medium shadow-sm" : "text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"}`}>
  ...
  return (
    <div className="min-h-screen flex bg-background">
      <aside className={`hidden md:flex flex-col bg-sidebar border-r border-sidebar-border transition-all duration-300 relative ${collapsed ? "w-16" : "w-60"}`}>
        {renderSidebar()}
      ...
      <aside className={`md:hidden fixed top-0 left-0 h-full w-64 bg-sidebar z-50 flex flex-col transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        {renderSidebar()}
      </aside>
