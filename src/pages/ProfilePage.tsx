...
export default function ProfilePage() {
  ...
  const inp: React.CSSProperties = {width:"100%",padding:"9px 12px",border:"1.5px solid #e5e7eb",borderRadius:8,fontSize:14,outline:"none",boxSizing:"border-box" as const};

  return (
    <div style={{padding:"16px 20px",fontFamily:"'Segoe UI',system-ui",maxWidth:900,margin:"0 auto"}}>
      <style>{`@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
      <div style={{display:"grid" as const,gridTemplateColumns:"1fr 1fr",gap:18}}>
        {/* Profile info */}
        <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:16,overflow:"hidden" as const,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
          <div style={{background:"linear-gradient(135deg,#0a2558,#1a3a6b)",padding:"20px 20px 40px"}}>
            <div style={{width:64,height:64,borderRadius:"50%",background:"#e2e8f0",display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const,marginBottom:10}}>
          ...
          <div style={{padding:"16px 20px",marginTop:-24}}>
            <div style={{background:"#fff",borderRadius:10,border:"1px solid #e5e7eb",padding:14,marginBottom:14}}>
              {roles.map(r=>(
                <span key={r} style={{display:"inline-flex" as const,alignItems:"center" as const,gap:4,padding:"3px 10px",borderRadius:20,background:"#eff6ff",color:"#1d4ed8",fontSize:11,fontWeight:700,marginRight:6,marginBottom:4}}>
            ...
            <button onClick={saveProfile} disabled={saving}
              style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"9px 20px",background:"#1e3a6b",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:700,width:"100%",justifyContent:"center" as const,opacity:saving?0.7:1}}>
        ...
        {/* Right column */}
        <div style={{display:"flex" as const,flexDirection:"column" as const,gap:14}}>
          {/* Account info */}
          <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:16,padding:16,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
            <div style={{fontSize:12,fontWeight:800,color:"#374151",marginBottom:12,display:"flex" as const,alignItems:"center" as const,gap:6}}><Mail style={{width:14,height:14,color:"#6b7280"}}/>Account Information</div>
            {[["Email",user?.email||"—"],["User ID",user?.id?.slice(0,16)+"..."||"—"],["Last Sign-In",user?.last_sign_in_at?new Date(user.last_sign_in_at).toLocaleDateString("en-KE"):"-"]].map(([l,v])=>(
              <div key={l} style={{display:"flex" as const,justifyContent:"space-between" as const,padding:"7px 0",borderBottom:"1px solid #f9fafb"}}>
          ...
          {/* Password reset */}
          <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:16,padding:16,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
            <div style={{fontSize:12,fontWeight:800,color:"#374151",marginBottom:8,display:"flex" as const,alignItems:"center" as const,gap:6}}><Key style={{width:14,height:14,color:"#6b7280"}}/>Password</div>
            ...
            <button onClick={sendReset} disabled={resetSending||resetSent}
              style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"9px 16px",background:resetSent?"#dcfce7":"#f9fafb",color:resetSent?"#15803d":"#374151",border:"1.5px solid #e5e7eb",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:700,width:"100%",justifyContent:"center" as const,opacity:resetSending?0.7:1}}>
          ...
          {activityLog.length>0&&(
            <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:16,overflow:"hidden" as const,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
              <div style={{padding:"10px 14px",background:"#f9fafb",borderBottom:"1px solid #e5e7eb",fontSize:11,fontWeight:800,color:"#374151",display:"flex" as const,alignItems:"center" as const,gap:6}}>
                <Clock style={{width:13,height:13,color:"#6b7280"}}/>Recent Activity
              </div>
              {activityLog.map((a,i)=>(
                <div key={i} style={{padding:"7px 14px",display:"flex" as const,justifyContent:"space-between" as const,borderBottom:"1px solid #f9fafb",fontSize:11}}>
