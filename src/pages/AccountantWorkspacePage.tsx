...
export default function AccountantWorkspacePage() {
  ...
  async function triggerManualSync() {
    ...
    setSyncing(false);
    if (!error) { showToast("[OK] Manual sync queued to Dynamics 365!"); fetchAll(); }
    else showToast("[X] Sync failed: " + error.message);
  }
  ...
  async function approveInvoiceMatch(id: string) {
    ...
    if (!error) { showToast("[OK] Invoice match approved!"); fetchAll(); }
    else showToast("[X] " + error.message);
  }
  ...
  async function approveBudgetOverride(id: string) {
    ...
    if (!error) { showToast("[OK] Budget override approved!"); fetchAll(); }
    else showToast("[X] " + error.message);
  }
  ...
  async function createQuotation() {
    ...
    if (!error) {
      showToast(`[OK] Quotation ${qNum} created!`);
  ...
  async function approvePayment(id: string) {
    ...
    if (!error) { showToast("[OK] Payment approved!"); fetchAll(); }
    else showToast("[X] " + error.message);
  }
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: "#6b7280", marginTop: 2 }}>EL5 MediProcure * Finance & Procurement Bridge * Dynamics 365 ERP</div>
            </div>
          </div>
        </div>
            ...
            {loading ? <div style={{ color: "#9ca3af", fontSize: 13 }}>Loading...</div> : (
              <>
                ...
                {invoiceMatches.filter(i => i.status === "pending").length === 0 && budgetAlerts.filter(b => !b.override_approved).length === 0 && (
                  <div style={{ textAlign: "center", padding: "20px 0", color: "#9ca3af", fontSize: 13 }}>[OK] No pending tasks</div>
                )}
              </>
            )}
          </div>
                ...
                <tbody>
                  {invoiceMatches.map(inv => (
                    <tr key={inv.id} style={{ transition: "background 0.15s" }}
                      ...
                      <td style={tblCell}><span style={{ fontSize: 16 }}>[OK]</span></td>
                      <td style={tblCell}>
                        {inv.status === "pending" ? (
                          <div style={{ display: "flex", gap: 6 }}>
        ...
        <div style={{ display: "grid", gap: 20 }}>
          <div style={card}>
            ...
            <div style={{ overflowX: "auto" }}>
                ...
                <tbody>
                  {budgetAlerts.map(b => (
                    <tr key={b.id}
                      ...
                      <td style={tblCell}><span style={{ fontSize: 16 }}>{b.override_approved ? "[OK]" : "[X]"}</span></td>
              ...
              {budgetAlerts.length === 0 && <div style={{ textAlign: "center", padding: "40px", color: "#9ca3af", fontSize: 14 }}>[OK] No budget alerts</div>}
            </div>
          </div>
        </div>
      )}
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                {[
                  ...
                  <button key={btn.type} onClick={async () => {
                    ...
                    showToast(`[OK] ${btn.label} queued!`); fetchAll();
              ...
              <tbody>
                {syncQueue.map(s => (
                  <tr key={s.id}
                    ...
                    <td style={tblCell}><span style={{ fontSize: 14 }}>{s.is_manual ? "[OK]" : ""}</span></td>
                    <td style={tblCell}><span style={{ fontSize: 14 }}>{s.gl_verified ? "[OK]" : " --"}</span></td>
          ...
          <div style={card}>
            ...
            <div style={{ background: "#f8fafc", borderRadius: 10, padding: "20px", border: "1.5px solid #e2e8f0" }}>
              ...
              <div style={{ fontSize: 12, color: "#6b7280", lineHeight: 1.7 }}>
                 ...
                 Embu Level 5 Hospital * ProcurBosse Health Procurement ERP
              </div>
            </div>
          </div>
        </div>
