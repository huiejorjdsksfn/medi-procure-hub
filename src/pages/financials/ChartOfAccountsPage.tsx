...
export default function ChartOfAccountsPage() {
  ...
  return (
    ...
    <div style={{padding:16,display:"flex" as const,flexDirection:"column" as const,gap:12,fontFamily:"'Segoe UI',system-ui"}}>
      ...
      {/* HEADER BAR */}
      <div style={{borderRadius:12,padding:"10px 18px",display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,background:"linear-gradient(90deg,#0f172a,#1e3a5f)"}}>
        <div>
        ...
        <div style={{display:"flex" as const,gap:8}}>
          ...
          {canManage&&<button onClick={()=>{setEditing(null);setForm({account_code:"",account_name:"",account_type:"Asset",category:"",parent_code:"",balance:"0",description:"",is_active:true});setShowNew(true);}} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"6px 16px",borderRadius:8,fontSize:12,fontWeight:700,border:"none",cursor:"pointer" as const,background:"rgba(255,255,255,0.92)",color:"#1e3a5f"}}><Plus style={{width:14,height:14}}/>New Account</button>}
        </div>
      </div>
      <div style={{display:"flex" as const,alignItems:"center" as const,gap:12,flexWrap:"wrap" as const}}>
        <div style={{position:"relative" as const}}><Search style={{position:"absolute" as const,left:12,top:"50%",transform:"translateY(-50%)",width:14,height:14,color:"#9ca3af"}}/>
        ...
        <div style={{display:"flex" as const,gap:4}}>
          {TYPES.map(t=>(
            <button key={t} onClick={()=>setTypeFilter(t)}
              style={{padding:"6px 10px",borderRadius:10,fontSize:12,fontWeight:600,textTransform:"capitalize" as const,border:"none",cursor:"pointer" as const,background:typeFilter===t?(TYPE_COLORS[t]||"#1a3a6b"):"#f3f4f6",color:typeFilter===t?"#fff":"#6b7280"}}>
              {t}
            </button>
          ))}
        </div>
        ...
        <div style={{position:"fixed" as const,inset:0,zIndex:1000,display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const}}>
          ...
          <div style={{position:"relative" as const,background:"#fff",borderRadius:16,boxShadow:"0 20px 60px rgba(0,0,0,0.3)",width:"min(580px,100%)",maxHeight:"90vh",display:"flex" as const,flexDirection:"column" as const}}>
            ...
            <div style={{flex:1,overflowY:"auto" as const,padding:"16px 20px"}}><div style={{display:"grid" as const,gridTemplateColumns:"1fr 1fr",gap:12}}>
              ...
              <div><label style={{display:"block" as const,marginBottom:4,fontSize:12,fontWeight:600,color:"#6b7280"}}>Account Type</label>
              ...
              <div style={{display:"flex" as const,alignItems:"center" as const,gap:8,paddingTop:16}}>
                ...
                <label htmlFor="isActive" style={{fontSize:14,fontWeight:600,color:"#374151",cursor:"pointer" as const}}>Active</label>
              </div>
            ...
            <div style={{display:"flex" as const,gap:8,justifyContent:"flex-end" as const,padding:"12px 20px",borderTop:"1px solid #e5e7eb",flexShrink:0}}>
              ...
              <button onClick={save} disabled={saving} style={{display:"flex" as const,alignItems:"center" as const,gap:8,padding:"8px 20px",borderRadius:10,color:"#fff",fontSize:14,fontWeight:700,border:"none",cursor:"pointer" as const,background:"#1e3a5f"}}>
