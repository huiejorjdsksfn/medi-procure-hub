  ...
  return (
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#f8fafc",minHeight:"100%",padding:16}}>
      ...
      <div  style={{background:"#fff",borderRadius:10,padding:"10px 14px",display:"flex" as const,gap:10,alignItems:"center" as const,marginBottom:12,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",flexWrap:"wrap" as const}}>
        ...
        <div style={{display:"flex" as const,gap:4}}>
          {["all","active","inactive","discontinued"].map(s=>(
            <button key={s} onClick={()=>setStatusFilter(s)} style={{padding:"4px 10px",borderRadius:20,fontSize:11,fontWeight:600,border:"none",cursor:"pointer" as const,textTransform:"capitalize" as const,background:statusFilter===s?"#1a3d12":"#f3f4f6",color:statusFilter===s?"#fff":"#6b7280"}}>
              {s}
            </button>
          ))}
        </div>
      ...
      <div style={{background:"#fff",borderRadius:10,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",overflow:"hidden" as const}}>
        ...
        <div style={{padding:"8px 14px",background:"#f9fafb",borderTop:"1px solid #e5e7eb",display:"flex" as const,gap:20,fontSize:11,color:"#6b7280",flexWrap:"wrap" as const}}>
