...
const ItemsPage = () => {
  ...
  const fetchItems = async () => {
    const { data } = await supabase
      .from("items")
      .select("*, item_categories(name), departments(name), suppliers(name)")
          ...
          <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label>Barcode</Label>
                  <Input value={form.barcode} onChange={(e) => setForm({ ...form, barcode: e.target.value })} placeholder="Scan or enter barcode" />
                </div>
                <div className="space-y-2">
                  <Label>SKU</Label>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={form.unit_of_measure} onValueChange={(v) => setForm({ ...form, unit_of_measure: v })}>
                    ...
                    <SelectContent>
                      {["piece","box","pack","bottle","roll","kg","liter","set","pair"].map((u) => <SelectItem key={u} value={u}>{u}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((item) => (
                <TableRow key={item.id} className="data-table-row">
                  ...
                  <TableCell className={`text-right font-medium ${item.quantity_in_stock <= item.reorder_level ? "text-destructive" : "text-foreground"}`}>
                    {item.quantity_in_stock}
                  </TableCell>
                  <TableCell className="text-right">{Number(item.unit_price).toFixed(2)}</TableCell>
                  <TableCell>
                    <span className={`text-xs px-2 py-1 rounded-full ${item.status === "active" ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}`}>
                      {item.status}
                    </span>
                  </TableCell>
