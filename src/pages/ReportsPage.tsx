...
export default function ReportsPage() {
  ...
  return (
      <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#f8fafc",minHeight:"100%",display:"flex" as const,flexDirection:"column" as const}}>
      {/* ── RETRO HEADER (VB6 style) ── */}
      <div style={{background:"#d4d0c8",borderBottom:"2px solid #999",padding:"6px 12px"}}>
        <div style={{display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,flexWrap:"wrap" as const,gap:8}}>
          {/* Logo + Title */}
          ...
          <div style={{background:"#ececec",border:"1px solid #aaa",borderRadius:4,padding:"6px 12px",display:"flex" as const,alignItems:"center" as const,gap:12,flexWrap:"wrap" as const}}>
          ...
          <div style={{display:"flex" as const,gap:8}}>
            ...
            <div style={{position:"relative" as const}}>
              ...
              {showDropdown && (
                <div style={{position:"absolute" as const,top:"100%",left:0,zIndex:50,width:224,maxHeight:256,overflowY:"auto" as const,background:"rgba(255,255,255,0.92)",border:"1px solid #aaa",borderRadius:8,boxShadow:"2px 2px 6px rgba(0,0,0,0.2)"}}>
                  {REPORT_TYPES.map(rt=>(
                    <button key={rt.id} onClick={()=>{setReportType(rt);setShowDropdown(false);}}
                      style={{display:"block" as const,width:"100%",textAlign:"left" as const,padding:"6px 12px",fontSize:12,background:"none",border:"none",cursor:"pointer" as const,color:reportType.id===rt.id?"#1d4ed8":"#1a1a2e",fontWeight:reportType.id===rt.id?700:400}}>
                      {rt.label}
                    </button>
                  ))}
                </div>
      ...
      <div style={{display:"flex" as const,gap:0,minHeight:400,flex:1}}>
        ...
        <div style={{flex:1,display:"flex" as const,flexDirection:"column" as const,background:"#d4d0c8"}}>
          ...
          <div style={{background:"#d4d0c8",border:"2px inset #aaa",margin:"6px 8px 4px",padding:"6px 10px",borderRadius:3}}>
            ...
            <div style={{display:"flex" as const,flexWrap:"wrap" as const,gap:12,alignItems:"flex-end" as const}}>
              <div style={{display:"flex" as const,alignItems:"center" as const,gap:8}}>
