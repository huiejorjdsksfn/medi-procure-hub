...
const PROCUREMENT_ROLES: { value: ProcurementRole; label: string; description: string }[] = [
  ...
  { value: "requisitioner", label: "Requisitioner", description: "Create requisitions" },
  ...
  { value: "warehouse_officer", label: "Warehouse Officer", description: "Receive goods" },
  { value: "inventory_manager", label: "Inventory Manager", description: "Manage items & stock" },
];
...
const UsersPage = () => {
  ...
  const assignRole = async () => {
    ...
    if (error) {
      if (error.message?.includes("duplicate")) toast({ title: "Already assigned", variant: "destructive" });
  ...
  const filtered = users.filter(u => (u.full_name || "").toLowerCase().includes(search.toLowerCase()) || (u.department || "").toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="space-y-4 animate-fade-in">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Button size="sm" variant="outline" onClick={() => exportToPDF(users.map(u => ({ name: u.full_name, department: u.department, roles: u.roles.join(", "), active: (u.is_active ?? true) ? "Yes" : "No" })), "Users Report", ["name","department","roles","active"])}><Download className="w-4 h-4 mr-1" /> PDF</Button>
              ...
              <TableRow key={u.id}>
                ...
                <TableCell>
                  <div className="flex gap-1">
                    <Button variant="outline" size="sm" onClick={() => { setRoleDialog(u); setSelectedRole(""); }}><KeyRound className="w-3 h-3 mr-1" /> Role</Button>
            ...
            <Select value={selectedRole} onValueChange={setSelectedRole}>
              ...
              <SelectContent>
                {PROCUREMENT_ROLES.map((r) => (
                  <SelectItem key={r.value} value={r.value}><span className="font-medium">{r.label}</span> <span className="text-xs text-muted-foreground ml-1">— {r.description}</span></SelectItem>
                ))}
              </SelectContent>
            </Select>
