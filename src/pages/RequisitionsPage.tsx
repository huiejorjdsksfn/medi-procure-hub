...
const RequisitionsPage = () => {
  ...
  const generateReqNumber = () => {
    const date = new Date();
    return `REQ-${date.getFullYear()}${String(date.getMonth() + 1).padStart(2, "0")}${String(date.getDate()).padStart(2, "0")}-${Math.random().toString(36).substring(2, 6).toUpperCase()}`;
  };
  ...
  const updateLineItem = (index: number, field: string, value: string) => {
    ...
    if (field === "item_id" && value) {
      ...
      if (item) {
        ...
        updated[index].unit_price = String(item.unit_price || 0);
      }
    }
                      ...
                      <TableBody>
                        {lineItems.map((li, i) => (
                          <TableRow key={i}>
                            ...
                            <TableCell>
                              <Input type="number" step="0.01" value={li.unit_price} onChange={(e) => updateLineItem(i, "unit_price", e.target.value)} />
                            </TableCell>
                            <TableCell className="text-right font-medium">
                              {((parseInt(li.quantity) || 0) * (parseFloat(li.unit_price) || 0)).toFixed(2)}
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                ...
                <TableRow key={req.id} className="data-table-row">
                  ...
                  <TableCell>
                    <span className={`text-xs px-2 py-1 rounded-full capitalize ${
                      req.priority === "urgent" ? "bg-destructive/10 text-destructive" :
                      req.priority === "high" ? "bg-warning/10 text-warning" :
                      "bg-muted text-muted-foreground"
                    }`}>{req.priority}</span>
                  </TableCell>
                  <TableCell className="text-right">{Number(req.total_amount).toFixed(2)}</TableCell>
                  <TableCell>
                    <span className={`text-xs px-2 py-1 rounded-full capitalize ${
                  ...
                  <TableCell>
                    <Button variant="ghost" size="sm" onClick={() => viewDetail(req)}>
          ...
          {detailDialog && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div><span className="text-muted-foreground">Total:</span> <span className="font-medium">{Number(detailDialog.total_amount).toFixed(2)}</span></div>
              ...
              {detailDialog.justification && (
                <div><Label className="text-muted-foreground">Justification</Label><p className="text-sm mt-1">{detailDialog.justification}</p></div>
              )}
              <Table>
                <TableHeader>
                  <TableRow>
              ...
              {isAdmin && detailDialog.status === "pending" && (
                <div className="flex gap-2 justify-end">
                  <Button variant="outline" onClick={() => updateStatus(detailDialog.id, "rejected")}>
                    <XCircle className="w-4 h-4 mr-1" /> Reject
                  </Button>
                  <Button onClick={() => updateStatus(detailDialog.id, "approved")}>
