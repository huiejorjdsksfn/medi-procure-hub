...
const TWILIO_MSG_SID = "REDACTED_TWILIO_MESSAGING_SID";
        ...
        {result&&(
          <div style={{marginTop:12,padding:"10px 14px",borderRadius:8,
            ...
            <div style={{fontSize:12,fontWeight:700,color:result.ok?"#166534":"#dc2626"}}>
              {result.ok?`[OK] Sent: ${result.sent}/${result.total}`:`[X] ${result.error||"Send failed"}`}
            </div>
...
export default function ReceptionPage() {
  ...
  async function saveVisitor(){
    ...
    if(!error){
      ...
      toast({title:"[OK] Visitor registered"});
  ...
  async function saveCall(){
    ...
    if(!error){setShowCF(false);setCF({...EC});toast({title:"[OK] Call logged"});}
    else toast({title:"Error",description:error.message,variant:"destructive"});
    setSaving(false);
  }
    ...
    <div style={{padding:"20px 24px",fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",maxWidth:1400,margin:"0 auto"}}>
        ...
        <div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              ...
              <div style={{fontSize:11,color:"#6b7280",marginTop:2}}>{hosName} * ProcurBosse * {getRoleWelcome(primaryRole)}</div>
            </div>
          </div>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,marginBottom:20}}>
        {[
          ...
          {label:"Currently In",value:visitors.filter(v=>v.status==="checked_in").length,icon:"[OK]",color:"#059669"},
          ...
          {showVF && (
            <div style={{background:"#f8fafc",borderRadius:12,padding:20,marginBottom:16,border:"1.5px solid #e2e8f0"}}>
              ...
              <div style={{display:"flex",justifyContent:"flex-end",gap:10,marginTop:14}}>
                ...
                <button onClick={saveVisitor} disabled={saving} style={BS(saving?"#9ca3af":"#0369a1")}>{saving?"Saving...":"[OK] Register Visitor"}</button>
              </div>
            </div>
          )}
