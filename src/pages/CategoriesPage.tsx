...
export default function CategoriesPage() {
  ...
  return (
    <div style={{padding:"16px 20px",fontFamily:"'Segoe UI',system-ui",minHeight:"100%"}}>
      ...
      {/* Header */}
      <div style={{background:"linear-gradient(90deg,#374151,#4b5563)",borderRadius:14,padding:"12px 20px",display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,marginBottom:14}}>
        <div>
        ...
        {canManage&&<button onClick={()=>{setEditing(null);setForm({name:"",description:"",parent_category:""});setShowNew(true);}}
          style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"7px 14px",background:"rgba(255,255,255,0.92)",color:"#374151",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:12,fontWeight:700}}>
          <Plus style={{width:13,height:13}}/>New Category
        </button>}
      </div>
      {/* Search */}
      <div style={{position:"relative" as const,maxWidth:320,marginBottom:14}}>
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search categories..."
          style={{width:"100%",padding:"8px 12px 8px 30px",border:"1.5px solid #e5e7eb",borderRadius:20,fontSize:12,outline:"none",boxSizing:"border-box" as const}}/>
      </div>
      {/* Table */}
      <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,overflow:"hidden" as const,boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          <thead>
            <tr style={{background:"#374151"}}>
              {["Category Name","Description","Parent Category","Actions"].map(h=>(
                <th key={h} style={{padding:"10px 14px",textAlign:"left" as const,fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.8)",textTransform:"uppercase" as const,letterSpacing:"0.05em"}}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading?(<tr><td colSpan={4} style={{padding:24,textAlign:"center" as const}}>
              <RefreshCw style={{width:16,height:16,color:"#d1d5db",animation:"spin 1s linear infinite",display:"block" as const,margin:"0 auto"}}/>
            </td></tr>):filtered.length===0?(<tr><td colSpan={4} style={{padding:32,textAlign:"center" as const,color:"#9ca3af"}}>No categories found</td></tr>):
            filtered.map((r,i)=>(
                ...
                <td style={{padding:"9px 14px"}}>
                  <div style={{display:"flex" as const,gap:6}}>
                    {canManage&&<button onClick={()=>{setEditing(r);setForm({name:r.name,description:r.description||"",parent_category:r.parent_category||""});setShowNew(true);}}
                      style={{padding:"4px 8px",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:6,cursor:"pointer" as const,lineHeight:0}}>
                      <Edit style={{width:12,height:12,color:"#2563eb"}}/>
                    </button>}
                    {hasRole("admin")&&<button onClick={()=>del(r.id)}
                      style={{padding:"4px 8px",background:"#fee2e2",border:"1px solid #fecaca",borderRadius:6,cursor:"pointer" as const,lineHeight:0}}>
      ...
      {/* Modal */}
      {showNew&&(
        <div style={{position:"fixed" as const,inset:0,background:"rgba(0,0,0,0.5)",zIndex:50,display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const}}>
          <div style={{background:"#fff",borderRadius:16,width:"min(420px,94vw)",padding:20,boxShadow:"0 24px 64px rgba(0,0,0,0.2)"}}>
            <div style={{display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,marginBottom:16}}>
              ...
              <button onClick={()=>{setShowNew(false);setEditing(null);}} style={{background:"none",border:"none",cursor:"pointer" as const}}><X style={{width:18,height:18,color:"#9ca3af"}}/></button>
            </div>
            {[["Category Name *","name"],["Description","description"],["Parent Category","parent_category"]].map(([l,k])=>(
              <div key={k} style={{marginBottom:12}}>
                <label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>{l}</label>
                <input value={(form as any)[k]||""} onChange={e=>setForm(p=>({...p,[k]:e.target.value}))} style={inp}/>
              </div>
            ))}
            <div style={{display:"flex" as const,gap:8,justifyContent:"flex-end" as const,marginTop:4}}>
              ...
              <button onClick={save} disabled={saving}
                style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"8px 20px",background:"#374151",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:700,opacity:saving?0.7:1}}>
