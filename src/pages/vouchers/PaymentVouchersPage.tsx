...
export default function PaymentVouchersPage() {
  ...
  async function createVoucher() {
    ...
    toast({ title: `[OK] Voucher ${vNum} created!` });
    setShowNew(false);
  ...
  async function updateStatus(id: string, status: string) {
    ...
    if (!error) { toast({ title: `[OK] Status updated to ${status}` }); fetchVouchers(); }
  ...
  async function bulkApprove() {
    ...
    toast({ title: `[OK] ${selected.length} vouchers approved!` });
  ...
  function printVoucher(v: PaymentVoucher) {
    ...
    <div class="header"><div><strong style="font-size:18px">EL5 MediProcure</strong><br/><span style="font-size:11px">Embu Level 5 Hospital * ProcurBosse</span></div><div style="text-align:right"><strong>PAYMENT VOUCHER</strong><br/>${v.voucher_number||""}</div></div>
    <table>
    ...
    <div style="margin-top:30px;font-size:10px;color:#9ca3af;text-align:center">Embu County Government * Embu Level 5 Hospital * Health Procurement Division * ProcurBosse Health Procurement ERP</div>
    </body></html>`);
    ...
    <div style={{padding:"20px 24px",fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",maxWidth:1400,margin:"0 auto"}}>
        ...
        <div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              ...
              <div style={{fontSize:12,color:"#6b7280",marginTop:2}}>Create * Approve * Print * Export * GL Sync * EL5 MediProcure</div>
            </div>
          </div>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:16,marginBottom:24}}>
        {[
          ...
          {label:"Approved Total",value:fmt(approvedTotal),color:"#22c55e",icon:"[OK]"},
