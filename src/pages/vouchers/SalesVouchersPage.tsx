...
export default function SalesVouchersPage() {
  ...
  const load = async () => {
    ...
    const rows=sv||[]; setRows(rows); setDepts(d||[]); setItems(it||[]);
      pageCache.set("sales_vouchers",rows);
    } catch(e:any) {
      const cached=pageCache.get<any[]>("sales_vouchers"); if(cached) setRows(cached);
      console.error("[SalesVouchers]",e);
  ...
  const printVoucher = (v:any) => {
    (printGenericVoucher as any)(v, "Sales Voucher", {
    ...
    <div style={{padding:16,display:"flex",flexDirection:"column",gap:16,fontFamily:"'Segoe UI',system-ui"}}>
      ...
      <div style={{borderRadius:16,padding:"12px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",background:"linear-gradient(90deg,#065f46,#0d9488)"}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <button onClick={exportExcel} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 12px",borderRadius:10,fontSize:12,fontWeight:600,border:"none",cursor:"pointer",background:"#64748b",color:"#fff"}}><Download style={{width:14,height:14}}/>Export</button>
