...
export default function NonConformancePage() {
  ...
  return (
    <div style={{padding:"16px 20px",fontFamily:"'Segoe UI',system-ui",minHeight:"100%"}}>
      ...
      <div style={{background:"linear-gradient(90deg,#7c2d12,#c2410c)",borderRadius:14,padding:"12px 20px",display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,marginBottom:14}}>
        ...
        <div style={{display:"flex" as const,gap:8}}>
          ...
          {canCreate&&<button onClick={()=>setShowNew(true)} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"7px 14px",background:"rgba(255,255,255,0.92)",color:"#7c2d12",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:12,fontWeight:700}}><Plus style={{width:13,height:13}}/>New NCR</button>}
        </div>
      </div>
      {/* Filters */}
      <div style={{display:"flex" as const,gap:8,flexWrap:"wrap" as const,marginBottom:14,alignItems:"center" as const}}>
        {[{id:"all",label:`All (${rows.length})`},{id:"open",label:`Open (${stats.open})`},{id:"under_review",label:`Under Review (${stats.under_review})`},{id:"closed",label:`Closed (${stats.closed})`}].map(f=>(
          <button key={f.id} onClick={()=>setStatusFilter(f.id)}
            style={{padding:"5px 12px",borderRadius:20,border:`1.5px solid ${statusFilter===f.id?"#c2410c":"#e5e7eb"}`,background:statusFilter===f.id?"#c2410c":"#fff",color:statusFilter===f.id?"#fff":"#374151",fontSize:11,fontWeight:700,cursor:"pointer" as const}}>
            {f.label}
          </button>
        ))}
      ...
      <div style={{background:"#fff",border:"1.5px solid #e5e7eb",borderRadius:12,overflow:"hidden" as const,boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          ...
          <tbody>
            {loading?(<tr><td colSpan={8} style={{padding:24,textAlign:"center" as const}}><RefreshCw style={{width:16,height:16,color:"#d1d5db",animation:"spin 1s linear infinite",display:"block" as const,margin:"0 auto"}}/></td></tr>):
            ...
            filtered.map((r,i)=>{
              ...
              return(<tr key={r.id} style={{borderBottom:"1px solid #f3f4f6",background:i%2===0?"#fff":"#fafafa"}}
                ...
                <td style={{padding:"9px 12px",fontWeight:600,color:"#1f2937",maxWidth:200,overflow:"hidden" as const,textOverflow:"ellipsis",whiteSpace:"nowrap" as const}}>{r.title}</td>
                ...
                <td style={{padding:"9px 12px"}}>
                  <div style={{display:"flex" as const,gap:4}}>
                    <button onClick={()=>setDetail(r)} style={{padding:"4px 8px",background:"#fff7ed",border:"1px solid #fed7aa",borderRadius:6,cursor:"pointer" as const,lineHeight:0}}><Eye style={{width:12,height:12,color:"#c2410c"}}/></button>
                    {r.status!=="closed"&&hasRole("admin")&&<button onClick={()=>updateStatus(r.id,"closed")} style={{padding:"3px 8px",background:"#dcfce7",border:"1px solid #bbf7d0",borderRadius:6,cursor:"pointer" as const,fontSize:9,fontWeight:700,color:"#15803d"}}>Close</button>}
      ...
      {detail&&(
        <div style={{position:"fixed" as const,inset:0,background:"rgba(0,0,0,0.4)",zIndex:50,display:"flex" as const,justifyContent:"flex-end" as const}} onClick={()=>setDetail(null)}>
          <div style={{width:"min(440px,100%)",background:"#fff",height:"100%",overflowY:"auto" as const,boxShadow:"-4px 0 24px rgba(0,0,0,0.15)"}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:"12px 16px",background:"linear-gradient(90deg,#7c2d12,#c2410c)",display:"flex" as const,alignItems:"center" as const,gap:8}}>
            ...
            <div style={{padding:16,display:"flex" as const,flexDirection:"column" as const,gap:10}}>
              ...
              {detail.status!=="closed"&&hasRole("admin")&&<button onClick={()=>{updateStatus(detail.id,"closed");setDetail(null);}} style={{padding:"9px",background:"#dcfce7",color:"#15803d",border:"1.5px solid #bbf7d0",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:700}}>Mark as Closed</button>}
            </div>
          </div>
        </div>
      )}
      ...
      {showNew&&(
        <div style={{position:"fixed" as const,inset:0,background:"rgba(0,0,0,0.5)",zIndex:50,display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const,padding:16}}>
          <div style={{background:"#fff",borderRadius:16,width:"min(620px,100%)",maxHeight:"90vh",overflowY:"auto" as const,boxShadow:"0 24px 64px rgba(0,0,0,0.2)"}}>
            <div style={{padding:"14px 18px",background:"linear-gradient(90deg,#7c2d12,#c2410c)",borderRadius:"16px 16px 0 0",display:"flex" as const,alignItems:"center" as const}}>
            ...
            <div style={{padding:18,display:"grid" as const,gridTemplateColumns:"1fr 1fr",gap:12}}>
              <div style={{gridColumn:"span 2"}}><label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>Title *</label><input value={form.title} onChange={e=>setForm(p=>({...p,title:e.target.value}))} style={inp}/></div>
              {[["NCR Date","ncr_date","date"],["Source","source","text"],["Supplier Name","supplier_name","text"],["Item Name","item_name","text"],["GRN Reference","grn_reference","text"],["Responsible Person","responsible_person","text"],["Target Date","target_date","date"]].map(([l,k,t])=>(
                <div key={k}><label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>{l}</label>
              ...
              <div><label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>Severity</label>
              ...
              {[["Description","description"],["Root Cause","root_cause"],["Corrective Action","corrective_action"],["Preventive Action","preventive_action"]].map(([l,k])=>(
                <div key={k} style={{gridColumn:"span 2"}}><label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>{l}</label>
            ...
            <div style={{padding:"12px 18px",borderTop:"1px solid #f3f4f6",display:"flex" as const,gap:8,justifyContent:"flex-end" as const}}>
              ...
              <button onClick={save} disabled={saving} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"8px 20px",background:"#c2410c",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:700,opacity:saving?0.7:1}}>
