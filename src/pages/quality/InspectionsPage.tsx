...
export default function InspectionsPage() {
  ...
  const inp: React.CSSProperties = {width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:13,outline:"none",boxSizing:"border-box" as const};

  return (
    <div style={{padding:"16px 20px",fontFamily:"'Segoe UI',system-ui",minHeight:"100%"}}>
      <style>{`@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
      <div style={{background:"linear-gradient(90deg,#134e4a,#0f766e)",borderRadius:14,padding:"12px 20px",display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,marginBottom:14}}>
        <div>
        ...
        <div style={{display:"flex" as const,gap:8}}>
          <button onClick={exportExcel} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"7px 14px",background:"#e2e8f0",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:12,fontWeight:600}}><Download style={{width:13,height:13}}/>Export</button>
          {canCreate&&<button onClick={()=>setShowNew(true)} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"7px 14px",background:"rgba(255,255,255,0.92)",color:"#134e4a",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:12,fontWeight:700}}><Plus style={{width:13,height:13}}/>New Inspection</button>}
        </div>
      </div>
      {/* KPI row */}
      <div style={{display:"grid" as const,gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:14}}>
        {[{label:"Passed",count:stats.pass,icon:CheckCircle,color:"#15803d"},{label:"Failed",count:stats.fail,icon:XCircle,color:"#dc2626"},{label:"Conditional",count:stats.conditional,icon:AlertTriangle,color:"#d97706"},{label:"Pending",count:stats.pending,icon:Clock,color:"#6b7280"}].map(s=>(
          <div key={s.label} style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:"12px 14px",display:"flex" as const,alignItems:"center" as const,gap:10,boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
            <div style={{width:34,height:34,borderRadius:9,background:`${s.color}18`,display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const}}><s.icon style={{width:16,height:16,color:s.color}}/></div>
      ...
      {/* Filters */}
      <div style={{display:"flex" as const,gap:8,flexWrap:"wrap" as const,marginBottom:14,alignItems:"center" as const}}>
        {["all","pass","fail","conditional","pending"].map(f=>(
          <button key={f} onClick={()=>setResultFilter(f)}
            style={{padding:"5px 12px",borderRadius:20,border:`1.5px solid ${resultFilter===f?"#0f766e":"#e5e7eb"}`,background:resultFilter===f?"#0f766e":"#fff",color:resultFilter===f?"#fff":"#374151",fontSize:11,fontWeight:700,cursor:"pointer" as const,textTransform:"capitalize" as const}}>
            {f==="all"?"All Results":f}
          </button>
        ))}
      ...
      <div style={{background:"#fff",border:"1.5px solid #e5e7eb",borderRadius:12,overflow:"hidden" as const,boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          ...
          <tbody>
            {loading?(<tr><td colSpan={9} style={{padding:24,textAlign:"center" as const}}><RefreshCw style={{width:16,height:16,color:"#d1d5db",animation:"spin 1s linear infinite",display:"block" as const,margin:"0 auto"}}/></td></tr>):
            filtered.length===0?(<tr><td colSpan={9} style={{padding:40,textAlign:"center" as const,color:"#9ca3af"}}>No inspections found</td></tr>):
            filtered.map((r,i)=>{
              ...
              return(<tr key={r.id} style={{borderBottom:"1px solid #f3f4f6",background:i%2===0?"#fff":"#f9fafb"}}
                ...
                <td style={{padding:"9px 12px",textAlign:"center" as const,color:"#374151"}}>{r.quantity_inspected||0}</td>
                <td style={{padding:"9px 12px",textAlign:"center" as const,color:"#15803d",fontWeight:700}}>{r.quantity_accepted||0}</td>
                <td style={{padding:"9px 12px",textAlign:"center" as const,color:"#dc2626",fontWeight:700}}>{r.quantity_rejected||0}</td>
                <td style={{padding:"9px 12px"}}><span style={{padding:"2px 9px",borderRadius:20,fontSize:10,fontWeight:700,textTransform:"capitalize" as const,background:rc.bg,color:rc.color}}>{r.result||"—"}</span></td>
                <td style={{padding:"9px 12px"}}>
                  <div style={{display:"flex" as const,gap:4}}>
                    <button onClick={()=>setDetail(r)} style={{padding:"4px 8px",background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:6,cursor:"pointer" as const,lineHeight:0}}><Eye style={{width:12,height:12,color:"#15803d"}}/></button>
                    {hasRole("admin")&&<button onClick={()=>del(r.id)} style={{padding:"4px 8px",background:"#fee2e2",border:"1px solid #fecaca",borderRadius:6,cursor:"pointer" as const,lineHeight:0}}><Trash2 style={{width:12,height:12,color:"#dc2626"}}/></button>}
                  </div>
                </td>
              </tr>);
            })}
      ...
      {/* Detail slide */}
      {detail&&(
        <div style={{position:"fixed" as const,inset:0,background:"rgba(0,0,0,0.4)",zIndex:50,display:"flex" as const,justifyContent:"flex-end" as const}} onClick={()=>setDetail(null)}>
          <div style={{width:"min(440px,100%)",background:"#fff",height:"100%",overflowY:"auto" as const,boxShadow:"-4px 0 24px rgba(0,0,0,0.15)"}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:"12px 16px",background:"linear-gradient(90deg,#134e4a,#0f766e)",display:"flex" as const,alignItems:"center" as const,gap:8}}>
              <span style={{fontSize:13,fontWeight:800,color:"#fff",flex:1}}>{detail.inspection_number}</span>
              <button onClick={()=>setDetail(null)} style={{background:"#e2e8f0",border:"none",borderRadius:5,padding:"4px 7px",cursor:"pointer" as const,color:"#fff",lineHeight:0}}><X style={{width:12,height:12}}/></button>
            </div>
            <div style={{padding:16,display:"flex" as const,flexDirection:"column" as const,gap:10}}>
              {[["Item",detail.item_name],["Supplier",detail.supplier_name||"—"],["Date",detail.inspection_date?new Date(detail.inspection_date).toLocaleDateString("en-KE"):"—"],["Inspector",detail.inspector_name||"—"],["Qty Inspected",String(detail.quantity_inspected||0)],["Qty Accepted",String(detail.quantity_accepted||0)],["Qty Rejected",String(detail.quantity_rejected||0)],["Result",detail.result],["GRN Reference",detail.grn_reference||"—"]].map(([l,v])=>(
                <div key={l} style={{display:"flex" as const,justifyContent:"space-between" as const,padding:"6px 0",borderBottom:"1px solid #f3f4f6"}}>
      ...
      {showNew&&(
        <div style={{position:"fixed" as const,inset:0,background:"rgba(0,0,0,0.5)",zIndex:50,display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const,padding:16}}>
          <div style={{background:"#fff",borderRadius:16,width:"min(640px,100%)",maxHeight:"90vh",overflowY:"auto" as const,boxShadow:"0 24px 64px rgba(0,0,0,0.2)"}}>
            <div style={{padding:"14px 18px",background:"linear-gradient(90deg,#134e4a,#0f766e)",borderRadius:"16px 16px 0 0",display:"flex" as const,alignItems:"center" as const,gap:8}}>
            ...
            <div style={{padding:18,display:"grid" as const,gridTemplateColumns:"1fr 1fr",gap:12}}>
              <div><label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>Inspection Date</label><input type="date" value={form.inspection_date} onChange={e=>setForm(p=>({...p,inspection_date:e.target.value}))} style={inp}/></div>
              <div><label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>Supplier</label>
              ...
              {[["Item Name *","item_name"],["GRN Reference","grn_reference"],["Inspector Name","inspector_name"]].map(([l,k])=>(
                <div key={k}><label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>{l}</label><input value={(form as any)[k]||""} onChange={e=>setForm(p=>({...p,[k]:e.target.value}))} style={inp}/></div>
              ))}
              {[["Qty Inspected","quantity_inspected"],["Qty Accepted","quantity_accepted"],["Qty Rejected","quantity_rejected"]].map(([l,k])=>(
                <div key={k}><label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>{l}</label><input type="number" min={0} value={(form as any)[k]||""} onChange={e=>setForm(p=>({...p,[k]:e.target.value}))} style={inp}/></div>
              ))}
              <div><label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>Result</label>
                <select value={form.result} onChange={e=>setForm(p=>({...p,result:e.target.value}))} style={inp}>
                  {["pending","pass","fail","conditional"].map(v=><option key={v} value={v} style={{textTransform:"capitalize" as const}}>{v}</option>)}
                </select>
              </div>
              <div style={{gridColumn:"span 2"}}><label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>Rejection Reason</label><input value={form.rejection_reason} onChange={e=>setForm(p=>({...p,rejection_reason:e.target.value}))} style={inp}/></div>
              <div style={{gridColumn:"span 2"}}><label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>Corrective Action</label><textarea value={form.corrective_action} onChange={e=>setForm(p=>({...p,corrective_action:e.target.value}))} rows={2} style={{...inp,resize:"vertical" as const,fontFamily:"inherit"}}/></div>
              <div style={{gridColumn:"span 2"}}><label style={{display:"block" as const,fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase" as const,marginBottom:4}}>Notes</label><textarea value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))} rows={2} style={{...inp,resize:"vertical" as const,fontFamily:"inherit"}}/></div>
            </div>
            <div style={{padding:"12px 18px",borderTop:"1px solid #f3f4f6",display:"flex" as const,gap:8,justifyContent:"flex-end" as const}}>
              <button onClick={()=>setShowNew(false)} style={{padding:"8px 16px",border:"1.5px solid #e5e7eb",borderRadius:8,cursor:"pointer" as const,fontSize:13}}>Cancel</button>
              <button onClick={save} disabled={saving} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"8px 20px",background:"#0f766e",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:700,opacity:saving?0.7:1}}>
