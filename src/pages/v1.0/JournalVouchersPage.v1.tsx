...
export default function JournalVouchersPage() {
  ...
  return (
    ...
    <div style={{padding:16,display:"flex" as const,flexDirection:"column" as const,gap:16,fontFamily:"'Segoe UI',system-ui"}}>
      ...
      <div style={{borderRadius:16,padding:"12px 20px",display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,background:"linear-gradient(90deg,#1e1b4b,#312e81)"}}>
        ...
        <div style={{display:"flex" as const,gap:8}}>
          ...
          {canApprove&&<button onClick={()=>setShowNew(true)} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"6px 16px",borderRadius:10,fontSize:12,fontWeight:700,border:"none",cursor:"pointer" as const,background:"rgba(255,255,255,0.92)",color:"#312e81"}}><Plus style={{width:14,height:14}}/>New Journal</button>}
        </div>
      </div>
      <div style={{position:"relative" as const,maxWidth:384}}><Search style={{position:"absolute" as const,left:12,top:"50%",transform:"translateY(-50%)",width:14,height:14,color:"#9ca3af"}}/>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search journals..."
          style={{width:"100%",paddingLeft:34,paddingRight:16,paddingTop:8,paddingBottom:8,borderRadius:10,border:"1.5px solid #e5e7eb",fontSize:14,outline:"none",boxSizing:"border-box" as const}}/></div>
        ...
        <table style={{width:"100%",fontSize:12,borderCollapse:"collapse"}}>
          <thead><tr style={{background:"#eef2ff"}}>
            {["Journal No.","Date","Reference","Narration","Debit","Credit","Balanced","Status","Actions"].map(h=>(
              <th key={h} style={{padding:"12px 16px",textAlign:"left" as const,fontWeight:700,color:"#4b5563",fontSize:10,textTransform:"uppercase"}}>{h}</th>))}
          </tr></thead>
          <tbody>
            ...
            filtered.map((r,i)=>(
              <tr key={r.id} style={{borderBottom:"1px solid #f3f4f6",background:i%2===0?"#fff":"#fafafa"}}>
                ...
                <td style={{padding:"10px 16px"}}><div style={{display:"flex" as const,gap:4}}>
                  <button onClick={()=>setDetail(r)} style={{padding:5,borderRadius:6,background:"#dbeafe",border:"none",cursor:"pointer" as const}}><Eye style={{width:12,height:12,color:"#2563eb"}}/></button>
                  <button onClick={()=>printVoucher(r)} style={{padding:5,borderRadius:6,background:"#dcfce7",border:"none",cursor:"pointer" as const}}><Printer style={{width:12,height:12,color:"#16a34a"}}/></button>
                  {canApprove&&r.status==="draft"&&<button onClick={()=>approve(r.id)} style={{padding:5,borderRadius:6,background:"#d1fae5",border:"none",cursor:"pointer" as const}} title="Approve"><CheckCircle style={{width:12,height:12,color:"#059669"}}/></button>}
      ...
      {showNew&&(
        <div style={{position:"fixed" as const,inset:0,zIndex:1000,display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const}}>
          <div style={{position:"fixed" as const,inset:0,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(4px)"}} onClick={()=>setShowNew(false)}/>
          <div style={{position:"relative" as const,background:"#fff",borderRadius:16,boxShadow:"0 20px 60px rgba(0,0,0,0.3)",width:"min(580px,100%)",maxHeight:"90vh",display:"flex" as const,flexDirection:"column" as const}}>
            <div style={{display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,padding:"14px 20px",borderBottom:"1px solid #e5e7eb",flexShrink:0}}>
              <h3 style={{fontWeight:900,color:"#1f2937",margin:0}}>New Journal Voucher</h3>
              <button onClick={()=>setShowNew(false)} style={{background:"none",border:"none",cursor:"pointer" as const}}><X style={{width:20,height:20,color:"#9ca3af"}}/></button>
            </div>
            ...
            <div style={{display:"grid" as const,gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
              <div><label style={{display:"block" as const,marginBottom:4,fontSize:12,fontWeight:600,color:"#6b7280"}}>Date *</label>
                <input type="date" value={form.journal_date} onChange={e=>setForm(p=>({...p,journal_date:e.target.value}))} style={{width:"100%",padding:"8px 12px",border:"1.5px solid #e5e7eb",borderRadius:8,fontSize:13,outline:"none",boxSizing:"border-box" as const}}/></div>
              <div><label style={{display:"block" as const,marginBottom:4,fontSize:12,fontWeight:600,color:"#6b7280"}}>Reference</label>
                <input value={form.reference} onChange={e=>setForm(p=>({...p,reference:e.target.value}))} style={{width:"100%",padding:"8px 12px",border:"1.5px solid #e5e7eb",borderRadius:8,fontSize:13,outline:"none",boxSizing:"border-box" as const}}/></div>
              <div><label style={{display:"block" as const,marginBottom:4,fontSize:12,fontWeight:600,color:"#6b7280"}}>Period</label>
                <input value={form.period} onChange={e=>setForm(p=>({...p,period:e.target.value}))} placeholder="e.g. Jan 2026" style={{width:"100%",padding:"8px 12px",border:"1.5px solid #e5e7eb",borderRadius:8,fontSize:13,outline:"none",boxSizing:"border-box" as const}}/></div>
              <div style={{gridColumn:"1/-1"}}><label style={{display:"block" as const,marginBottom:4,fontSize:12,fontWeight:600,color:"#6b7280"}}>Narration *</label>
                <textarea value={form.narration} onChange={e=>setForm(p=>({...p,narration:e.target.value}))} rows={2} style={{width:"100%",padding:"8px 12px",border:"1.5px solid #e5e7eb",borderRadius:8,fontSize:13,outline:"none",boxSizing:"border-box" as const}}/></div>
            </div>
            {/* Entries table */}
            <div>
              <div style={{display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,marginBottom:8}}>
                ...
                <button onClick={()=>setEntries(p=>[...p,{account_code:"",account_name:"",debit:"",credit:"",description:""}])}
                  style={{display:"flex" as const,alignItems:"center" as const,gap:4,fontSize:12,fontWeight:600,color:"#4f46e5",background:"none",border:"none",cursor:"pointer" as const}}>
                ...
                <tbody>
                  {entries.map((e,i)=>(
                    <tr key={i} style={{borderBottom:"1px solid #f3f4f6"}}>
                      ...
                      <td style={{padding:"4px"}}><button onClick={()=>setEntries(p=>p.filter((_,j)=>j!==i))} style={{color:"#f87171",background:"none",border:"none",cursor:"pointer" as const}}><X style={{width:12,height:12}}/></button></td>
                    </tr>
                  ))}
            ...
            <div style={{display:"flex" as const,gap:8,justifyContent:"flex-end" as const,padding:"12px 20px",borderTop:"1px solid #e5e7eb",flexShrink:0}}>
              <button onClick={()=>setShowNew(false)} style={{padding:"8px 16px",borderRadius:10,border:"1.5px solid #e5e7eb",background:"#fff",fontSize:14,cursor:"pointer" as const}}>Cancel</button>
              <button onClick={save} disabled={saving||!isBalanced}
                style={{display:"flex" as const,alignItems:"center" as const,gap:8,padding:"8px 20px",borderRadius:10,color:"#fff",fontSize:14,fontWeight:700,border:"none",cursor:"pointer" as const,background:"#312e81"}}>
