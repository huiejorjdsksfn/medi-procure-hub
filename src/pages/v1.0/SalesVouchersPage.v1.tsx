...
export default function SalesVouchersPage() {
  ...
  return (
    ...
    <div style={{padding:16,display:"flex" as const,flexDirection:"column" as const,gap:16,fontFamily:"'Segoe UI',system-ui"}}>
      ...
      <div style={{borderRadius:16,padding:"12px 20px",display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,background:"linear-gradient(90deg,#065f46,#0d9488)"}}>
        ...
        <div style={{display:"flex" as const,gap:8}}>
          ...
          {canCreate&&<button onClick={()=>setShowNew(true)} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"6px 16px",borderRadius:10,fontSize:12,fontWeight:700,border:"none",cursor:"pointer" as const,background:"rgba(255,255,255,0.92)",color:"#065f46"}}><Plus style={{width:14,height:14}}/>New Sale</button>}
        </div>
      </div>
      <div style={{position:"relative" as const,maxWidth:384}}><Search style={{position:"absolute" as const,left:12,top:"50%",transform:"translateY(-50%)",width:14,height:14,color:"#9ca3af"}}/>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search..." style={{width:"100%",paddingLeft:34,paddingRight:16,paddingTop:8,paddingBottom:8,borderRadius:10,border:"1.5px solid #e5e7eb",fontSize:14,outline:"none",boxSizing:"border-box" as const}}/></div>
      ...
      {showNew&&(
        <div style={{position:"fixed" as const,inset:0,zIndex:1000,display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const}}>
          ...
          <div style={{position:"relative" as const,background:"#fff",borderRadius:16,boxShadow:"0 20px 60px rgba(0,0,0,0.3)",width:"min(700px,100%)",maxHeight:"90vh",display:"flex" as const,flexDirection:"column" as const}}>
            ...
            <div style={{flex:1,overflowY:"auto" as const,padding:"16px 20px"}}><div style={{display:"grid" as const,gridTemplateColumns:"1fr 1fr",gap:12}}>
              ...
              <div><label style={{display:"block" as const,marginBottom:4,fontSize:12,fontWeight:600,color:"#6b7280"}}>Customer Type</label>
              ...
              <div><label style={{display:"block" as const,marginBottom:4,fontSize:12,fontWeight:600,color:"#6b7280"}}>Payment Method</label>
                <select value={form.payment_method} onChange={e=>setForm(p=>({...p,payment_method:e.target.value}))}
                  style={{width:"100%",padding:"8px 12px",border:"1.5px solid #e5e7eb",borderRadius:8,fontSize:13,outline:"none",boxSizing:"border-box" as const}}>
                  {["Cash","MPESA","Insurance","Cheque","EFT","Credit"].map(m=><option key={m}>{m}</option>)}
                </select></div>
              <div><label style={{display:"block" as const,marginBottom:4,fontSize:12,fontWeight:600,color:"#6b7280"}}>Tax Rate (%)</label>
                <input type="number" value={form.tax_rate} onChange={e=>setForm(p=>({...p,tax_rate:e.target.value}))} style={{width:"100%",padding:"8px 12px",border:"1.5px solid #e5e7eb",borderRadius:8,fontSize:13,outline:"none",boxSizing:"border-box" as const}}/></div>
            ...
            <div>
              <div style={{display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,marginBottom:8}}>
                ...
                <button onClick={()=>setLineItems(p=>[...p,{item_id:"",item_name:"",qty:"1",rate:"",amount:""}])} style={{display:"flex" as const,alignItems:"center" as const,gap:4,fontSize:12,fontWeight:600,color:"#0f766e",background:"none",border:"none",cursor:"pointer" as const}}><Plus style={{width:12,height:12}}/>Add</button>
              </div>
                  ...
                  {lineItems.map((it,i)=>(
                    <tr key={i} style={{borderBottom:"1px solid #f3f4f6"}}>
                      <td style={{padding:"4px"}}>
                        <select value={it.item_id} onChange={e=>updateLine(i,"item_id",e.target.value)} style={{width:"100%",padding:"4px 8px",borderRadius:6,border:"1px solid #e5e7eb",fontSize:12,outline:"none",boxSizing:"border-box" as const}}>
            ...
            <div style={{display:"flex" as const,gap:8,justifyContent:"flex-end" as const,padding:"12px 20px",borderTop:"1px solid #e5e7eb",flexShrink:0}}>
              ...
              <button onClick={save} disabled={saving} style={{display:"flex" as const,alignItems:"center" as const,gap:8,padding:"8px 20px",borderRadius:10,color:"#fff",fontSize:14,fontWeight:700,border:"none",cursor:"pointer" as const,background:"#065f46"}}>
