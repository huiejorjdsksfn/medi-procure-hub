  ...
  return (
      ...
      <div style={{display:"flex" as const,alignItems:"flex-start" as const,justifyContent:"space-between" as const,marginBottom:20,flexWrap:"wrap",gap:12}}>
        <div style={{display:"flex" as const,alignItems:"center" as const,gap:10}}>
          <div style={{width:44,height:44,borderRadius:10,background:"linear-gradient(135deg,#0369a1,#0284c7)",display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const}}>
        ...
        <div style={{display:"flex" as const,gap:8}}>
          <button onClick={exportXLSX} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"9px 14px",background:"#f3f4f6",border:"1.5px solid #e5e7eb",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:600}}>
          ...
          {canManage&&<button onClick={()=>openNew()} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"9px 18px",background:"linear-gradient(135deg,#0369a1,#0284c7)",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:800,boxShadow:"0 2px 8px rgba(3,105,161,0.3)"}}>
      ...
      <div style={{display:"flex" as const,gap:8,marginBottom:14,flexWrap:"wrap"}}>
        {[{id:"all",label:"All"},{id:"active",label:"Active"},{id:"expired",label:"Expired"},{id:"draft",label:"Draft"},{id:"terminated",label:"Terminated"}].map(f=>(
          <button key={f.id} onClick={()=>setStFilter(f.id)} style={{padding:"6px 14px",borderRadius:20,border:`1.5px solid ${stFilter===f.id?"#0369a1":"#e5e7eb"}`,background:stFilter===f.id?"#0369a1":"#fff",color:stFilter===f.id?"#fff":"#374151",fontSize:12,fontWeight:700,cursor:"pointer" as const}}>
            ...
            {loading?[1,2,3].map(i=>(
              ...
              return(
                <tr key={r.id} style={{borderBottom:"1px solid #f9fafb",cursor:"pointer" as const}}
                  ...
                  <td style={{padding:"12px 14px",fontSize:13,fontWeight:600,color:"#111827",maxWidth:180,overflow:"hidden" as const,textOverflow:"ellipsis",whiteSpace:"nowrap" as const}} onClick={()=>setDetail(r)}>{r.title}</td>
                  ...
                  <td style={{padding:"12px 14px"}} onClick={()=>setDetail(r)}>
                    <span style={{display:"inline-flex" as const,alignItems:"center" as const,gap:4,fontSize:11,fontWeight:700,padding:"3px 9px",borderRadius:20,background:cfg.bg,color:cfg.color}}>
                      <SIcon style={{width:9,height:9}}/> {cfg.label}
                    </span>
                  </td>
                  <td style={{padding:"12px 14px"}} onClick={e=>e.stopPropagation()}>
                    {canManage&&<button onClick={()=>openNew(r)} style={{padding:"5px 11px",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:5,cursor:"pointer" as const,fontSize:11,fontWeight:700,color:"#1d4ed8"}}>Edit</button>}
                  </td>
                </tr>
              );
            })}
          ...
          <div style={{background:"#fff",borderRadius:14,width:"min(680px,100%)",maxHeight:"92vh",overflowY:"auto" as const,boxShadow:"0 24px 64px rgba(0,0,0,0.25)"}}>
            ...
            <div style={{padding:20,display:"flex" as const,flexDirection:"column" as const,gap:14}}>
              ...
              <div style={{display:"flex" as const,gap:8,justifyContent:"flex-end" as const,paddingTop:8,borderTop:"1px solid #f3f4f6"}}>
                ...
                <button onClick={save} disabled={saving} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"9px 22px",background:"linear-gradient(135deg,#0369a1,#0284c7)",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:800}}>
        ...
        <div style={{position:"fixed" as const,inset:0,background:"rgba(0,0,0,0.4)",zIndex:400,display:"flex" as const,justifyContent:"flex-end" as const}} onClick={()=>setDetail(null)}>
          <div style={{width:"min(440px,100%)",background:"#fff",height:"100%",overflowY:"auto" as const,boxShadow:"-4px 0 24px rgba(0,0,0,0.15)"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{padding:18,display:"flex" as const,flexDirection:"column" as const,gap:12}}>
              ...
              {canManage&&<button onClick={()=>{setDetail(null);openNew(detail);}} style={{width:"100%",padding:"11px",background:"linear-gradient(135deg,#0a2558,#1a3a6b)",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:14,fontWeight:800}}>Edit Contract</button>}
            </div>
          </div>
        </div>
      )}
