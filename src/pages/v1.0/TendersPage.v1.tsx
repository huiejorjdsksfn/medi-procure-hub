...
const LBL = ({c,children}:{c?:string;children:any}) => <div style={{fontSize:12,fontWeight:700,color:c||"#374151",marginBottom:5,textTransform:"uppercase" as const,letterSpacing:"0.05em"}}>{children}</div>;
  ...
  return (
      ...
      <div style={{display:"flex" as const,alignItems:"flex-start" as const,justifyContent:"space-between" as const,marginBottom:20,flexWrap:"wrap",gap:12}}>
        <div style={{display:"flex" as const,alignItems:"center" as const,gap:10}}>
          <div style={{width:44,height:44,borderRadius:10,background:"linear-gradient(135deg,#1F6090,#2980b9)",display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const}}>
        ...
        <div style={{display:"flex" as const,gap:8}}>
          <button onClick={exportXLSX} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"9px 14px",background:"#f3f4f6",border:"1.5px solid #e5e7eb",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:600,color:"#374151"}}>
          ...
          {canManage&&<button onClick={()=>openNew()} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"9px 18px",background:"linear-gradient(135deg,#1F6090,#2980b9)",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:800,boxShadow:"0 2px 8px rgba(31,96,144,0.3)"}}>
      ...
      {/* Status chips */}
      <div style={{display:"flex" as const,gap:8,marginBottom:16,flexWrap:"wrap"}}>
        {[{id:"all",label:"All",cnt:rows.length},
          ...Object.entries(STATUS_CFG).map(([id,c])=>({id,label:c.label,cnt:counts[id]||0}))].map(f=>(
          <button key={f.id} onClick={()=>setStFilter(f.id)} style={{padding:"6px 14px",borderRadius:20,border:`1.5px solid ${stFilter===f.id?"#1a3a6b":"#e5e7eb"}`,background:stFilter===f.id?"#1a3a6b":"#fff",color:stFilter===f.id?"#fff":"#374151",fontSize:12,fontWeight:700,cursor:"pointer" as const}}>
                ...
                <tr key={r.id} style={{borderBottom:"1px solid #f9fafb",cursor:"pointer" as const}}
                  ...
                  <td style={{padding:"12px 14px"}} onClick={e=>e.stopPropagation()}>
                    <div style={{display:"flex" as const,gap:4,flexWrap:"wrap"}}>
                      <button onClick={()=>setDetail(r)} style={{padding:"4px 9px",background:"#f3f4f6",border:"1px solid #e5e7eb",borderRadius:5,cursor:"pointer" as const,fontSize:11,fontWeight:600}}><Eye style={{width:10,height:10,display:"inline" as const}}/></button>
                      {canManage&&<button onClick={()=>openNew(r)} style={{padding:"4px 9px",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:5,cursor:"pointer" as const,fontSize:11,fontWeight:700,color:"#1d4ed8"}}>Edit</button>}
                      {canManage&&r.status==="draft"&&<button onClick={()=>publish(r)} style={{display:"flex" as const,alignItems:"center" as const,gap:3,padding:"4px 9px",background:"#dcfce7",border:"1px solid #bbf7d0",borderRadius:5,cursor:"pointer" as const,fontSize:11,fontWeight:700,color:"#15803d"}}>
                        <CheckCircle style={{width:9,height:9}}/> Publish
                      </button>}
                      {canManage&&r.status==="published"&&<button onClick={()=>close_(r)} style={{padding:"4px 9px",background:"#fef3c7",border:"1px solid #fde68a",borderRadius:5,cursor:"pointer" as const,fontSize:11,fontWeight:700,color:"#92400e"}}>Close</button>}
                    </div>
                  </td>
                </tr>
              );
        ...
        <div style={{position:"fixed" as const,inset:0,background:"rgba(0,0,0,0.55)",zIndex:500,display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const,padding:16}}>
          <div style={{background:"#fff",borderRadius:14,width:"min(680px,100%)",maxHeight:"92vh",overflowY:"auto" as const,boxShadow:"0 24px 64px rgba(0,0,0,0.25)"}}>
            <div style={{padding:"14px 18px",background:"linear-gradient(135deg,#0a2558,#1a3a6b)",borderRadius:"14px 14px 0 0",display:"flex" as const,gap:10,alignItems:"center" as const,position:"sticky" as const,top:0,zIndex:1}}>
              ...
              <button onClick={()=>{setShowNew(false);setEditing(null);}} style={{background:"rgba(255,255,255,0.15)",border:"none",borderRadius:6,padding:"4px 7px",cursor:"pointer" as const,color:"#fff",lineHeight:0}}><X style={{width:13,height:13}}/></button>
            </div>
            <div style={{padding:20,display:"flex" as const,flexDirection:"column" as const,gap:14}}>
              ...
              <div style={{display:"flex" as const,gap:8,justifyContent:"flex-end" as const,paddingTop:8,borderTop:"1px solid #f3f4f6"}}>
                <button onClick={()=>{setShowNew(false);setEditing(null);}} style={{padding:"9px 18px",background:"#f3f4f6",border:"1px solid #e5e7eb",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:600}}>Cancel</button>
                <button onClick={save} disabled={saving} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"9px 22px",background:"linear-gradient(135deg,#1F6090,#2980b9)",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:800}}>
        ...
        <div style={{position:"fixed" as const,inset:0,background:"rgba(0,0,0,0.4)",zIndex:400,display:"flex" as const,justifyContent:"flex-end" as const}} onClick={()=>setDetail(null)}>
          <div style={{width:"min(460px,100%)",background:"#fff",height:"100%",overflowY:"auto" as const,boxShadow:"-4px 0 24px rgba(0,0,0,0.15)"}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:"14px 16px",background:"linear-gradient(135deg,#0a2558,#1a3a6b)",display:"flex" as const,gap:8,alignItems:"center" as const}}>
              ...
              <button onClick={()=>setDetail(null)} style={{background:"rgba(255,255,255,0.15)",border:"none",borderRadius:5,padding:"4px 6px",cursor:"pointer" as const,color:"#fff",lineHeight:0}}><X style={{width:12,height:12}}/></button>
            </div>
            <div style={{padding:18,display:"flex" as const,flexDirection:"column" as const,gap:14}}>
              <div><div style={{fontSize:18,fontWeight:800,color:"#111827",lineHeight:1.3}}>{detail.title}</div>
                <span style={{fontSize:11,fontWeight:700,padding:"3px 9px",borderRadius:20,background:sc(detail.status).bg,color:sc(detail.status).color,marginTop:6,display:"inline-block" as const}}>{sc(detail.status).label}</span>
              </div>
              ...
              {detail.description&&<div><div style={{fontSize:12,fontWeight:700,color:"#9ca3af",marginBottom:5,textTransform:"uppercase" as const}}>Description</div>
                <p style={{fontSize:14,color:"#374151",lineHeight:1.7,margin:0}}>{detail.description}</p>
              </div>}
              {detail.evaluation_criteria&&<div><div style={{fontSize:12,fontWeight:700,color:"#9ca3af",marginBottom:5,textTransform:"uppercase" as const}}>Evaluation Criteria</div>
                <p style={{fontSize:14,color:"#374151",lineHeight:1.7,margin:0}}>{detail.evaluation_criteria}</p>
              </div>}
              {canManage&&detail.status==="draft"&&<button onClick={()=>{publish(detail);setDetail(null);}} style={{width:"100%",padding:"11px",background:"#15803d",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:14,fontWeight:800,display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const,gap:6}}>
