...
function BackupInner() {
  ...
  return (
      <div style={{padding:24,maxWidth:896,margin:"0 auto",display:"flex" as const,flexDirection:"column" as const,gap:24, fontFamily:"'Segoe UI',system-ui,sans-serif", background:"transparent", minHeight:"calc(100vh-100px)" }}>
      {/* Header card */}
      <div style={{borderRadius:16}}>
        <div style={{display:"flex" as const,alignItems:"flex-start" as const,justifyContent:"space-between" as const}}>
          <div>
            <h1 style={{fontSize:18,fontWeight:900,color:"#1f2937",display:"flex" as const,alignItems:"center" as const,gap:8}}>
          ...
          <button onClick={runBackup} disabled={running}
            style={{display:"flex" as const,alignItems:"center" as const,gap:8,padding:"10px 20px",borderRadius:10,fontSize:14,fontWeight:700,color:"#fff",border:"none",cursor:"pointer" as const, background:running?"#9ca3af":"linear-gradient(135deg,#1a3a6b,#1d4a87)" }}>
            {running ? (
        ...
        {running && (
          <div style={{marginTop:16,display:"flex" as const,flexDirection:"column" as const,gap:8}}>
            <div style={{display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,fontSize:12,color:"#6b7280"}}>
              <span>Processing: <strong>{currentTable}</strong></span>
              <span>{progress}%</span>
            </div>
            <div style={{width:"100%",height:8,borderRadius:4,background:"#f3f4f6",overflow:"hidden" as const}}>
      ...
      <div style={{borderRadius:16}}>
        ...
        <div style={{padding:16,display:"grid" as const,gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:8}}>
          {BACKUP_TABLES.map(t => (
            <div key={t} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"6px 10px",borderRadius:8,fontSize:12, background:"#f8fafc", border:"1px solid #e5e7eb" }}>
              <Database style={{width:12,height:12,flexShrink:0, color:"#1a3a6b" }} />
              <span style={{overflow:"hidden" as const,textOverflow:"ellipsis",whiteSpace:"nowrap" as const,color:"#4b5563"}}>{t}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Backup history */}
      <div style={{borderRadius:16}}>
        <div style={{padding:"14px 20px",borderBottom:"1px solid #f3f4f6",display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const}}>
          <h2 style={{fontSize:12,fontWeight:900,textTransform:"uppercase",letterSpacing:"0.1em",color:"#4b5563"}}>Backup History</h2>
          <button onClick={loadJobs} style={{padding:5,borderRadius:6,background:"transparent",border:"none",cursor:"pointer" as const}}>
        ...
        <div style={{}}>
          {loading ? (
            ...
            <div key={j.id} style={{display:"flex" as const,alignItems:"center" as const,gap:16,padding:"14px 20px"}}>
              <div style={{width:32,height:32,borderRadius:10,display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const,flexShrink:0, background:j.status==="completed"?"#d1fae5":j.status==="failed"?"#fee2e2":"#fef3c7" }}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:12,fontWeight:600,color:"#1f2937",overflow:"hidden" as const,textOverflow:"ellipsis",whiteSpace:"nowrap" as const}}>{j.label}</div>
                <div style={{fontSize:10,color:"#9ca3af",display:"flex" as const,alignItems:"center" as const,gap:8,marginTop:2}}>
      ...
      <div style={{borderRadius:16}}>
        ...
        <div style={{padding:20,display:"grid" as const,gridTemplateColumns:"1fr 1fr",gap:16}}>
          <div style={{display:"flex" as const,flexDirection:"column" as const,gap:12}}>
            ...
            {["Excel (XLSX)","CSV (per table)","JSON dump","SQL Script"].map(fmt=>(
              <label key={fmt} style={{display:"flex" as const,alignItems:"center" as const,gap:8,cursor:"pointer" as const}}>
          ...
          <div style={{display:"flex" as const,flexDirection:"column" as const,gap:12}}>
            ...
            {["Daily at midnight","Weekly (Sunday)","Monthly (1st)","Manual only"].map(sch=>(
              <label key={sch} style={{display:"flex" as const,alignItems:"center" as const,gap:8,cursor:"pointer" as const}}>
          ...
          <div style={{gridColumn:"1/-1",paddingTop:12,borderTop:"1px solid #f3f4f6",display:"flex" as const,flexDirection:"column" as const,gap:8}}>
            ...
            <div style={{display:"flex" as const,flexWrap:"wrap",gap:8}}>
              {["All Tables","Procurement Only","Finance Only","Users & Roles","System Settings","Audit Logs","Quality"].map(scope=>(
                <label key={scope} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"6px 12px",borderRadius:10,cursor:"pointer" as const,background:backupScope.includes(scope)?"#f0f9ff":"#f9fafb",border:`1px solid ${backupScope.includes(scope)?"#bae6fd":"#e5e7eb"}`}}>
            ...
            <div style={{marginTop:16,display:"flex" as const,gap:12}}>
              <button onClick={runBackup} disabled={running} style={{display:"flex" as const,alignItems:"center" as const,gap:8,padding:"8px 16px",borderRadius:10,fontSize:12,fontWeight:700,color:"#fff",border:"none",cursor:"pointer" as const,background:"linear-gradient(135deg,#1a3a6b,#1d4a87)"}}>
                <Download style={{width:14,height:14}}/> Full Backup Now
              </button>
              <button style={{display:"flex" as const,alignItems:"center" as const,gap:8,padding:"8px 16px",borderRadius:10,fontSize:12,fontWeight:700,border:"none",cursor:"pointer" as const,background:"#f0fdf4",color:"#15803d",border:"1px solid #86efac"}}>
                <Shield style={{width:14,height:14}}/> Verify Last Backup
              </button>
              <button style={{display:"flex" as const,alignItems:"center" as const,gap:8,padding:"8px 16px",borderRadius:10,fontSize:12,fontWeight:700,border:"none",cursor:"pointer" as const,background:"#fff7ed",color:"#c2410c",border:"1px solid #fed7aa"}}>
      ...
      <div style={{borderRadius:16}}>
        <div style={{padding:"14px 20px",borderBottom:"1px solid #f3f4f6"}}>
          <h2 style={{fontSize:12,fontWeight:900,textTransform:"uppercase",letterSpacing:"0.1em",color:"#4b5563",display:"flex" as const,alignItems:"center" as const,gap:8}}>
        ...
        <div style={{padding:20}}>
          ...
          <div style={{display:"flex" as const,alignItems:"center" as const,gap:12}}>
            <label style={{display:"flex" as const,alignItems:"center" as const,gap:8,padding:"10px 16px",borderRadius:10,fontSize:12,fontWeight:700,cursor:"pointer" as const,border:"none",background:"#f0f9ff",border:"1px solid #bae6fd",color:"#0369a1"}}>
              <FileSpreadsheet style={{width:16,height:16}}/>
              Upload Backup File (.xlsx)
              <input type="file" accept=".xlsx,.csv,.json" style={{display:"none" as const}} onChange={()=>{}}/>
            </label>
            <button style={{display:"flex" as const,alignItems:"center" as const,gap:8,padding:"10px 16px",borderRadius:10,fontSize:12,fontWeight:700,border:"none",cursor:"pointer" as const,background:"#fef2f2",color:"#ef4444",border:"1px solid #fca5a5"}} onClick={handleRestore}>
