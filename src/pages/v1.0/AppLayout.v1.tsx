      ...
      <div style={{padding:collapsed&&!forMobile?"12px 0":"14px 12px",borderBottom:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",gap:9,justifyContent:collapsed&&!forMobile?"center":"flex-start",flexShrink:0}}>
        ...
        {(!collapsed||forMobile)&&(
          <div style={{minWidth:0}}>
            <div style={{fontSize:11.5,fontWeight:800,color:"#fff",letterSpacing:"0.02em",lineHeight:1.2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{sysName}</div>
            <div style={{fontSize:8.5,color:"rgba(255,255,255,0.4)",marginTop:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{hospitalName}</div>
          </div>
        )}
          ...
          return (
            <div key={m.id}>
              <button onClick={()=>{
                ...
                style={{display:"flex",alignItems:"center",width:"100%",padding:collapsed&&!forMobile?"10px 0":"8px 12px",justifyContent:collapsed&&!forMobile?"center":"flex-start",gap:8,border:"none",cursor:"pointer",textAlign:"left",borderLeft:isCurrent?`3px solid ${m.color}`:"3px solid transparent",background:isCurrent?"rgba(255,255,255,0.12)":"transparent",transition:"all 0.14s"}}
        ...
        <header style={{height:50,flexShrink:0,background:"linear-gradient(135deg,#0a2558 0%,#1a3a6b 60%,#1d4a87 100%)",borderBottom:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",padding:"0 14px",gap:8,boxShadow:"0 2px 12px rgba(0,0,0,0.2)",zIndex:90,position:"relative"}}>
          ...
          {isMobile&&(
            <div style={{display:"flex",alignItems:"center",gap:5}}>
              ...
              <span style={{fontSize:11,fontWeight:800,color:"#fff",whiteSpace:"nowrap"}}>{sysName}</span>
            </div>
          )}
          ...
          <div style={{display:"flex",alignItems:"center",gap:3,marginLeft:"auto",flexShrink:0}}>
            ...
            <div style={{position:"relative"}} ref={userMenuRef}>
              <button onClick={()=>setUserMenuOpen(v=>!v)}
                ...
                {!isMobile&&<div style={{textAlign:"left",lineHeight:1}}>
                  <div style={{fontSize:10.5,fontWeight:700,color:"#fff",maxWidth:80,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{profile?.full_name?.split(" ")[0]||"User"}</div>
                ...
                <div style={{position:"absolute",right:0,top:"calc(100% + 5px)",minWidth:200,background:"#fff",boxShadow:"0 8px 28px rgba(0,0,0,0.18)",border:"1px solid #e5e7eb",borderRadius:9,overflow:"hidden",zIndex:300}}>
                  ...
                  {[
                    ...
                    <button key={item.path} onClick={()=>{navigate(item.path);setUserMenuOpen(false);}}
                      style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"9px 14px",border:"none",background:"transparent",cursor:"pointer",textAlign:"left",fontSize:12,color:"#374151"}}
                      onMouseEnter={e=>(e.currentTarget as HTMLElement).style.background="#f9fafb"}
                      onMouseLeave={e=>(e.currentTarget as HTMLElement).style.background="transparent"}>
                  ...
                  <div style={{borderTop:"1px solid #f3f4f6"}}>
                    <button onClick={()=>{signOut();navigate("/login");}}
                      style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"9px 14px",border:"none",background:"transparent",cursor:"pointer",textAlign:"left",fontSize:12,color:"#dc2626"}}
