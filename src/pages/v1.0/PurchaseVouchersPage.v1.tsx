...
export default function PurchaseVouchersPage() {
  ...
  return (
    ...
    <div style={{padding:16,display:"flex" as const,flexDirection:"column" as const,gap:16,fontFamily:"'Segoe UI',system-ui"}}>
      ...
      <div style={{borderRadius:16,padding:"12px 20px",display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,background:"linear-gradient(90deg,#7c2d12,#b45309)"}}>
        ...
        <div style={{display:"flex" as const,gap:8}}>
          ...
          {canCreate&&<button onClick={()=>setShowNew(true)} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"6px 16px",borderRadius:10,fontSize:12,fontWeight:700,border:"none",cursor:"pointer" as const,background:"rgba(255,255,255,0.92)",color:"#7c2d12"}}><Plus style={{width:14,height:14}}/>New Voucher</button>}
        </div>
      </div>
            ...
            filtered.map((r,i)=>(
              <tr key={r.id} style={{borderBottom:"1px solid #f3f4f6",background:i%2===0?"#fff":"#fafafa"}}>
                ...
                <td style={{padding:"10px 16px"}}><div style={{display:"flex" as const,gap:4}}>
                  ...
                  {canApprove&&r.status==="pending"&&<button onClick={()=>approve(r.id)} style={{padding:5,borderRadius:6,background:"#d1fae5",border:"none",cursor:"pointer" as const}} title="Approve"><CheckCircle style={{width:12,height:12,color:"#059669"}}/></button>}
        ...
        <div style={{position:"fixed" as const,inset:0,zIndex:1000,display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const}}>
          ...
          <div style={{position:"relative" as const,background:"#fff",borderRadius:16,boxShadow:"0 20px 60px rgba(0,0,0,0.3)",width:"min(700px,100%)",maxHeight:"90vh",display:"flex" as const,flexDirection:"column" as const}}>
            ...
            <div style={{flex:1,overflowY:"auto" as const,padding:"16px 20px"}}><div style={{display:"grid" as const,gridTemplateColumns:"1fr 1fr",gap:12}}>
              <div><label style={{display:"block" as const,marginBottom:4,fontSize:12,fontWeight:600,color:"#6b7280"}}>Supplier *</label>
              ...
              {[["Invoice No.","invoice_number"],["Date","voucher_date","date"],["Due Date","due_date","date"],["PO Reference","po_reference"],["Expense Account","expense_account"],["Tax Rate (%)","tax_rate","number"]].map(([l,k,t])=>(
                <div key={k}><label style={{display:"block" as const,marginBottom:4,fontSize:12,fontWeight:600,color:"#6b7280"}}>{l}</label>
                  <input type={t||"text"} value={(form as any)[k]||""} onChange={e=>setForm(p=>({...p,[k]:e.target.value}))} style={{width:"100%",padding:"8px 12px",border:"1.5px solid #e5e7eb",borderRadius:8,fontSize:13,outline:"none",boxSizing:"border-box" as const}}/></div>
              ))}
            </div>
            {/* Line items */}
            <div>
              <div style={{display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,marginBottom:8}}>
                ...
                <button onClick={()=>setItems(p=>[...p,{description:"",qty:"1",rate:"",amount:""}])} style={{display:"flex" as const,alignItems:"center" as const,gap:4,fontSize:12,fontWeight:600,background:"none",border:"none",cursor:"pointer" as const}}><Plus style={{width:12,height:12}}/>Add</button>
              </div>
                ...
                <tbody>
                  {items.map((it,i)=>(
                    <tr key={i} style={{borderBottom:"1px solid #f3f4f6"}}>
                      <td style={{padding:"4px"}}><input value={it.description} onChange={e=>updateItem(i,"description",e.target.value)} style={{width:"100%",padding:"4px 8px",borderRadius:6,border:"1px solid #e5e7eb",fontSize:12,outline:"none",boxSizing:"border-box" as const}}/></td>
            ...
            <div style={{display:"flex" as const,gap:8,justifyContent:"flex-end" as const,padding:"12px 20px",borderTop:"1px solid #e5e7eb",flexShrink:0}}>
              ...
              <button onClick={save} disabled={saving} style={{display:"flex" as const,alignItems:"center" as const,gap:8,padding:"8px 20px",borderRadius:10,color:"#fff",fontSize:14,fontWeight:700,border:"none",cursor:"pointer" as const,background:"#7c2d12"}}>
