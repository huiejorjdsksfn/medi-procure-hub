...
export default function AuditLogPage() {
  ...
  return (
    <div style={{padding:"16px 20px",fontFamily:"'Segoe UI',system-ui,sans-serif",minHeight:"calc(100vh - 60px)"}}>
      ...
      <div style={{background:"linear-gradient(90deg,#374151,#4b5563)",borderRadius:14,padding:"12px 20px",display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,marginBottom:14,boxShadow:"0 4px 16px rgba(55,65,81,0.3)"}}>
        ...
        <div style={{display:"flex" as const,gap:8}}>
          <button onClick={fetchLogs} disabled={loading}
            style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"6px 14px",background:"rgba(255,255,255,0.15)",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:12,fontWeight:600}}>
      ...
      {/* Filters */}
      <div style={{background:"rgba(8,20,55,0.82)",backdropFilter:"blur(14px)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:"10px 16px",display:"flex" as const,flexWrap:"wrap",gap:10,marginBottom:14,alignItems:"center" as const}}>
      ...
      {/* Table */}
      <div style={{background:"rgba(8,20,55,0.82)",backdropFilter:"blur(14px)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:14,overflow:"hidden" as const}}>
        {loading?(
              ...
              <thead>
                <tr style={{background:"rgba(10,25,70,0.9)"}}>
                  {["#","Date & Time","User","Action","Module","Record ID","IP Address","Details"].map(h=>(
                    <th key={h} style={{padding:"10px 12px",textAlign:"left" as const,fontSize:9,fontWeight:800,color:"rgba(255,255,255,0.45)",textTransform:"uppercase",letterSpacing:"0.07em",whiteSpace:"nowrap" as const}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
        ...
        {totalPages>1&&(
          <div style={{display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,padding:"8px 16px",borderTop:"1px solid rgba(255,255,255,0.07)"}}>
            ...
            <div style={{display:"flex" as const,gap:4}}>
              <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1}
                style={{padding:"4px 10px",background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",color:"rgba(255,255,255,0.6)",borderRadius:6,cursor:"pointer" as const,fontSize:12,opacity:page===1?0.4:1}}>‹</button>
              ...
              <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={page===totalPages}
                style={{padding:"4px 10px",background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.12)",color:"rgba(255,255,255,0.6)",borderRadius:6,cursor:"pointer" as const,fontSize:12,opacity:page===totalPages?0.4:1}}>›</button>
            </div>
          </div>
        )}
      </div>
