      ...
      <div style={{width:220,flexShrink:0,background:"#111827",display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
        ...
        <div style={{padding:"12px 16px",borderTop:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",gap:9}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:9,color:"rgba(255,255,255,0.35)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{profile?.email||""}</div>
          </div>
        </div>
      </div>
              ...
              <div key={msg.id} className={`msg-row${isActive?" active":""}`}
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",gap:6}}>
                    <span style={{fontSize:12,fontWeight:msg.is_read?500:700,color:"#111827",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:160}}>{msg.from_name||"System"}</span>
                    <span style={{fontSize:9.5,color:"#9ca3af",flexShrink:0}}>{timeStr(msg.created_at)}</span>
                  </div>
                  <div style={{fontSize:11.5,fontWeight:msg.is_read?400:600,color:msg.is_read?"#374151":"#111827",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginTop:1}}>{msg.subject}</div>
                  <div style={{fontSize:10.5,color:"#9ca3af",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginTop:2}}>{msg.body.slice(0,60)}...</div>
                </div>
        ...
        {selected&&!composing&&(
          <>
            ...
            <div style={{flex:1,overflowY:"auto",padding:"24px"}}>
              <div style={{maxWidth:720,fontSize:13,color:"#374151",lineHeight:1.8,whiteSpace:"pre-wrap"}}>{selected.body}</div>
              {selected.action_url&&(
            ...
            {replyMode&&(
              <div style={{padding:"16px 24px",borderTop:"1px solid #e5e7eb",background:"#f9fafb"}}>
                ...
                <textarea value={replyBody} onChange={e=>setReplyBody(e.target.value)} rows={5}
                  placeholder="Write your reply..." style={{...inp,resize:"none",background:"#fff"}}/>
          ...
          {[
            ...
            return(
              <button key={i} className="ctx-item" onClick={it.action}
                style={{display:"flex",alignItems:"center",gap:10,padding:"7px 14px",border:"none",background:"transparent",cursor:"pointer",width:"100%",textAlign:"left",fontSize:12,color:it.danger?"#dc2626":"#374151"}}>
