...
export default function FacilitiesPage() {
  ...
  return (
    <div style={{ padding:16, display:"flex", flexDirection:"column", gap:12, fontFamily:"'Segoe UI',system-ui,sans-serif", background:"#f8fafc", minHeight:"100%" }}>
      ...
      {/* Header */}
      <div style={{ borderRadius:12, background:"linear-gradient(90deg,#0a2558,#1a3a6b)", padding:"12px 18px", display:"flex", alignItems:"center", justifyContent:"space-between", boxShadow:"0 4px 16px rgba(26,58,107,0.3)" }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={load} style={{ padding:"6px 10px", borderRadius:8, background:"rgba(255,255,255,0.15)", color:"#fff", border:"none", cursor:"pointer" }}>
            <RefreshCw style={{ width:14, height:14, animation:loading?"spin 1s linear infinite":"none" }}/>
          </button>
          {isAdmin && (
            <button onClick={openNew} style={{ display:"flex", alignItems:"center", gap:6, padding:"6px 14px", borderRadius:8, background:"#fff", color:"#0a2558", border:"none", cursor:"pointer", fontSize:12, fontWeight:700 }}>
            ...
            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:12 }}>
              ...
              {isAdmin && (
                <div style={{ display:"flex", gap:4 }}>
                  <button onClick={() => openEdit(f)} style={{ padding:5, borderRadius:6, background:"#eff6ff", color:"#2563eb", border:"none", cursor:"pointer" }}>
                    <Edit2 style={{ width:12, height:12 }}/>
                  </button>
                  {!f.is_main && (
                    <button onClick={() => del(f)} style={{ padding:5, borderRadius:6, background:"#fee2e2", color:"#dc2626", border:"none", cursor:"pointer" }}>
          ...
          <div style={{ position:"relative", background:"#fff", borderRadius:16, width:"min(680px,97%)", boxShadow:"0 24px 80px rgba(0,0,0,0.2)", border:"1px solid #e2e8f0", overflow:"hidden" }}>
            ...
            <div style={{ padding:20, display:"flex", flexDirection:"column", gap:12 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                <div>
                  ...
                  <div style={{ display:"flex", gap:6 }}>
                    <input type="color" value={form.primary_color||"#0a2558"} onChange={e => setForm(p=>({...p,primary_color:e.target.value}))} style={{ width:38,height:34,borderRadius:6,border:"1.5px solid #e2e8f0",cursor:"pointer",padding:2 }}/>
                ...
                <div>
                  ...
                  <div style={{ display:"flex", gap:6 }}>
                    <input type="color" value={form.accent_color||"#C45911"} onChange={e => setForm(p=>({...p,accent_color:e.target.value}))} style={{ width:38,height:34,borderRadius:6,border:"1.5px solid #e2e8f0",cursor:"pointer",padding:2 }}/>
              ...
              <div style={{ display:"flex", gap:16 }}>
                <label style={{ display:"flex", alignItems:"center", gap:7, cursor:"pointer", fontSize:12, color:"#374151", fontWeight:500 }}>
              ...
              <div style={{ display:"flex", gap:8, justifyContent:"flex-end", paddingTop:8, borderTop:"1px solid #e2e8f0" }}>
                <button onClick={() => setShowForm(false)} style={{ padding:"8px 18px", borderRadius:8, border:"1px solid #e2e8f0", background:"#fff", color:"#64748b", cursor:"pointer", fontSize:13 }}>Cancel</button>
