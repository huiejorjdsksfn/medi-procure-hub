/**
 * ProcurBosse — System Settings Page v4.0 (NUCLEAR REWRITE)
 * Tabbed design linked to system_settings table
 */
...
import {
  Settings, Save, Building2, Mail, Phone, Shield,
  Palette, Sliders, Printer, Server, Eye, EyeOff, CheckCircle, AlertTriangle
} from "lucide-react";
...
const Toggle = ({ on, onChange }: { on: boolean; onChange: (v: boolean) => void }) => (
  <button onClick={() => onChange(!on)} style={{ background:"none",border:"none",cursor:"pointer",padding:0,flexShrink:0 }}>
...
const Card = ({ title, sub, color, icon: Icon, onSave, saving, children }: any) => (
...
function SettingsInner() {
  ...
  const btn = (c: string) => ({ padding:"8px 18px",borderRadius:8,border:"none",background:c,color:"#fff",fontWeight:700 as const,fontSize:13,cursor:"pointer" });
  ...
  return (
    <div style={{ minHeight:"100vh",background:"linear-gradient(135deg,#070d1a 0%,#0d1b35 50%,#0a1225 100%)",color:"#f1f5f9",fontFamily:"var(--font-sans)" }}>
      ...
      {/* Header */}
      <div style={{ background:"rgba(79,70,229,0.12)",borderBottom:"1px solid rgba(79,70,229,0.25)",padding:"12px 22px",display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:50,backdropFilter:"blur(10px)" }}>
        ...
        <div>
          ...
          <div style={{ fontSize:11,color:"#64748b" }}>All changes saved to Supabase · propagate to all users in real-time</div>
        </div>
        <div style={{ marginLeft:"auto",display:"flex",gap:8,alignItems:"center" }}>
          ...
          <button onClick={saveAll} disabled={saving} style={{ padding:"8px 18px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#4f46e5,#7c3aed)",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:6 }}>
            <Save style={{ width:13,height:13 }} />{saving?"Saving…":"Save All Settings"}
          </button>
        </div>
      </div>

      <div style={{ display:"grid",gridTemplateColumns:"190px 1fr",minHeight:"calc(100vh - 60px)" }}>
        ...
        <div style={{ background:"rgba(0,0,0,0.35)",borderRight:"1px solid rgba(255,255,255,0.05)",paddingTop:8 }}>
          ...
          <div style={{ margin:"12px",padding:"10px",background:"rgba(79,70,229,0.1)",borderRadius:8,border:"1px solid rgba(79,70,229,0.2)" }}>
            ...
            <div style={{ fontSize:10,color:"#64748b",lineHeight:1.5 }}>Settings propagate to all users instantly via Supabase Realtime.</div>
          </div>
        </div>
        ...
        <div style={{ padding:"22px 28px",overflowY:"auto",maxHeight:"calc(100vh - 60px)" }}>
          ...
          {tab==="hospital" && (
            <Card title="Hospital Information" sub="Core details used in all documents and prints" color="#0078d4" icon={Building2}
              onSave={() => save(["hospital_name","county_name","department_name","system_name","hospital_address","po_box","hospital_phone","hospital_email","doc_footer","currency_symbol","vat_rate","logo_url","seal_url"])} saving={saving}>
              {[
          ...
          {tab==="email" && (
            <Card title="Email & SMTP" sub="Configure SMTP for email delivery" color="#059669" icon={Mail}
              ...
              <Row label="Use TLS" color="#059669"><Toggle on={(get("smtp_tls","true"))!=="false"} onChange={v=>set("smtp_tls",v?"true":"false")} /></Row>
              <Row label="Enable SMTP" sub="Use SMTP for all email delivery" color="#059669"><Toggle on={get("smtp_enabled")==="true"} onChange={v=>set("smtp_enabled",v?"true":"false")} /></Row>
              {testResult && (
          ...
          {tab==="sms" && (
            <Card title="SMS / Twilio" sub="SMS notifications via Twilio" color="#7c3aed" icon={Phone}
              ...
              <Row label="Enable Twilio SMS" sub="Activate SMS via Twilio" color="#7c3aed"><Toggle on={get("twilio_enabled")==="true"} onChange={v=>set("twilio_enabled",v?"true":"false")} /></Row>
              {[
              ...
              <div style={{ margin:"8px 0",padding:"8px 12px",background:"rgba(124,58,237,0.1)",borderRadius:8,fontSize:11,color:"#a78bfa" }}>
                Get credentials at <a href="https://www.twilio.com/console" target="_blank" rel="noreferrer" style={{ color:"#818cf8" }}>twilio.com/console</a>. Credentials stored securely in Supabase.
              </div>
              <Row label="SMS on PO Approval" color="#7c3aed"><Toggle on={(get("sms_on_po_approve","true"))!=="false"} onChange={v=>set("sms_on_po_approve",v?"true":"false")} /></Row>
              <Row label="SMS on Req Approval" color="#7c3aed"><Toggle on={(get("sms_on_req_approve","true"))!=="false"} onChange={v=>set("sms_on_req_approve",v?"true":"false")} /></Row>
              <Row label="SMS on Low Stock" color="#7c3aed"><Toggle on={(get("sms_on_low_stock","true"))!=="false"} onChange={v=>set("sms_on_low_stock",v?"true":"false")} /></Row>
              <Row label="SMS on Payment" color="#7c3aed"><Toggle on={(get("sms_on_payment","true"))!=="false"} onChange={v=>set("sms_on_payment",v?"true":"false")} /></Row>
            </Card>
          )}

          {tab==="security" && (
            <Card title="Security & Access Control" sub="IP restriction, sessions, authentication" color="#dc2626" icon={Shield}
              onSave={() => save(["ip_restriction_enabled","allow_all_private","log_all_ips","revoke_on_ip_change","force_network_check","allowed_ips","session_timeout","max_login_attempts","audit_logins"])} saving={saving}>
              {[
          ...
          {tab==="appearance" && (
            <Card title="Appearance & UI" sub="Brand colours, theme" color="#8b5cf6" icon={Palette}
            ...
            <Card title="Module Toggles" sub="Enable or disable ERP modules system-wide" color="#0369a1" icon={Sliders}
              ...
              {[
                ...
                <Row key={f.k} label={f.l} sub={f.s} color={f.k==="maintenance_mode"?"#dc2626":"#0369a1"}>
                  <Toggle on={(get(f.k,"true"))!=="false"} onChange={v=>set(f.k,v?"true":"false")} />
                </Row>
              ))}
            </Card>
          )}
          ...
          {tab==="print" && (
            <Card title="Print & Documents" sub="Font, paper size, letterhead options" color="#C45911" icon={Printer}
              onSave={() => save(["print_font","print_font_size","paper_size","show_logo_print","show_stamp","show_watermark","print_confidential"])} saving={saving}>
              ...
              <Row label="Font Size (pt)" color="#C45911"><input value={get("print_font_size","11")} onChange={e=>set("print_font_size",e.target.value)} style={{...inp,width:70}} type="number" min="8" max="16" /></Row>
              ...
              <Row label="Show Logo on Prints" color="#C45911"><Toggle on={(get("show_logo_print","true"))!=="false"} onChange={v=>set("show_logo_print",v?"true":"false")} /></Row>
              <Row label="Show Official Stamp Box" color="#C45911"><Toggle on={(get("show_stamp","true"))!=="false"} onChange={v=>set("show_stamp",v?"true":"false")} /></Row>
              <Row label="Show Watermark" color="#C45911"><Toggle on={get("show_watermark")==="true"} onChange={v=>set("show_watermark",v?"true":"false")} /></Row>
              <Row label="Confidential Notice" sub="Adds 'Private and Confidential' to docs" color="#C45911"><Toggle on={(get("print_confidential","true"))!=="false"} onChange={v=>set("print_confidential",v?"true":"false")} /></Row>
            </Card>
          )}
          ...
          {tab==="system" && (
            <Card title="System Configuration" sub="Timezone, date format, uploads" color="#374151" icon={Server}
              ...
              <Row label="System Version" color="#374151"><span style={{ fontFamily:"var(--font-mono)",fontWeight:700,color:"#818cf8" }}>v2.0.0 — ProcurBosse</span></Row>
              <Row label="Date Format" color="#374151">
              ...
              <Row label="Timezone" color="#374151"><input value={get("timezone","Africa/Nairobi")} onChange={e=>set("timezone",e.target.value)} style={{...inp,width:200}} /></Row>
              <Row label="Max Upload (MB)" color="#374151"><input value={get("max_upload_mb","25")} onChange={e=>set("max_upload_mb",e.target.value)} style={{...inp,width:80}} type="number" /></Row>
              <Row label="Default User Role" color="#374151">
              ...
              <Row label="Allow Self-Registration" sub="Users can sign up without admin" color="#374151">
