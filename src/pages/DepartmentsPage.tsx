...
export default function DepartmentsPage() {
  ...
  return (
    <div style={{padding:"16px 20px",fontFamily:"'Segoe UI',system-ui",minHeight:"100%"}}>
      ...
      <div style={{background:"linear-gradient(90deg,#4338ca,#6366f1)",borderRadius:14,padding:"12px 20px",display:"flex" as const,alignItems:"center" as const,justifyContent:"space-between" as const,marginBottom:14}}>
        <div>
        ...
        <div style={{display:"flex" as const,gap:8}}>
          <button onClick={exportExcel} style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"7px 14px",background:"#e2e8f0",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:12,fontWeight:600}}>
          ...
          {canManage&&<button onClick={()=>{setEditing(null);setForm({name:"",code:"",description:"",head_of_department:"",phone:"",email:"",budget_center:""});setShowNew(true);}}
            style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"7px 14px",background:"rgba(255,255,255,0.92)",color:"#4338ca",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:12,fontWeight:700}}>
                ...
                <td style={{padding:"9px 14px"}}>
                  <div style={{display:"flex" as const,gap:6}}>
                    {canManage&&<button onClick={()=>{setEditing(r);setForm({name:r.name,code:r.code||"",description:r.description||"",head_of_department:r.head_of_department||"",phone:r.phone||"",email:r.email||"",budget_center:r.budget_center||""});setShowNew(true);}}
                      style={{padding:"4px 8px",background:"#eef2ff",border:"1px solid #c7d2fe",borderRadius:6,cursor:"pointer" as const,lineHeight:0}}>
      ...
      {showNew&&(
        <div style={{position:"fixed" as const,inset:0,background:"rgba(0,0,0,0.5)",zIndex:50,display:"flex" as const,alignItems:"center" as const,justifyContent:"center" as const}}>
          <div style={{background:"#fff",borderRadius:16,width:"min(520px,94vw)",padding:20,boxShadow:"0 24px 64px rgba(0,0,0,0.2)"}}>
            ...
            <div style={{display:"flex" as const,gap:8,justifyContent:"flex-end" as const,marginTop:16}}>
              ...
              <button onClick={save} disabled={saving}
                style={{display:"flex" as const,alignItems:"center" as const,gap:6,padding:"8px 20px",background:"#4338ca",color:"#fff",border:"none",borderRadius:8,cursor:"pointer" as const,fontSize:13,fontWeight:700,opacity:saving?0.7:1}}>
