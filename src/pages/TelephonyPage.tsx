/**
 * EL5 MediProcure v5.8 — Telephony & Call Center
...
export default function TelephonyPage() {
  ...
  async function makeCall() {
    if (!dialNum.trim()) { showToast("⚠️ Enter a number or extension"); return; }
    ...
    <div style={{padding:"20px 24px",fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",maxWidth:1400,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:12}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div>
            ...
            <div style={{fontSize:12,color:"#6b7280",marginTop:2}}>EL5H · Twilio +16812972643 · ERP SIP Integration · v5.8</div>
          </div>
        </div>
      ...
      {metrics&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:12,marginBottom:20}}>
        {[
          ...
          {label:"Avg Duration",value:fmt(Math.round(metrics.avgDuration)),color:"#06b6d4",icon:"⏱️"},
        ].map((k,i)=>(
        ...
        <div style={{display:"grid",gridTemplateColumns:"340px 1fr",gap:20}}>
          <div style={card}>
            <div style={{fontWeight:800,fontSize:16,color:"#0f172a",marginBottom:20,textAlign:"center"}}>
              {activeCall ? (activeCall.status==="connected"?"📞 On Call":"🔔 Calling…") : "☎️ Softphone"}
            </div>
            ...
            {activeCall ? (
              <div style={{textAlign:"center"}}>
                ...
                <div style={{fontSize:12,color:"#9ca3af"}}>{muted&&"🔇 Muted · "}{onHold&&"⏸️ On Hold · "}From ext. {fromExt}</div>
              </div>
            ) : (
              <>
                {/* Dialpad */}
              ...
              <tbody>
                {extensions.map(e=>(
                  <tr key={e.id} onMouseEnter={ev=>(ev.currentTarget as HTMLElement).style.background="#f8fafc"} onMouseLeave={ev=>(ev.currentTarget as HTMLElement).style.background=""}>
                    ...
                    <td style={{padding:"10px 14px",borderBottom:"1px solid #f8fafc",fontSize:16}}>{e.voicemail_enabled?"✅":"❌"}</td>
      ...
      {tab==="ivr"&&isAdmin&&(
        <div style={{display:"grid",gridTemplateColumns:"280px 1fr",gap:20}}>
          <div style={card}>
            <div style={{fontWeight:700,fontSize:14,color:"#0f172a",marginBottom:14}}>🎛️ IVR Menus</div>
            {menus.map(m=>(
            ...
            {showIVREdit ? (
              <>
                ...
                <div style={{display:"flex",gap:10,marginTop:16}}>
                  <button onClick={async()=>{await TelephonyAPI.saveIVRMenu(showIVREdit);showToast("✅ IVR menu saved!");loadAll();}} style={btn("linear-gradient(135deg,#7c3aed,#6d28d9)")}>💾 Save Menu</button>
          ...
          {voicemails.length===0 ? (
            ...
            <div key={vm.id} style={{padding:"14px 16px",borderRadius:10,border:"1.5px solid #e2e8f0",marginBottom:10,background:"#fafcff"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  {vm.audio_url&&<button style={{...btn("#7c3aed"),padding:"6px 12px",fontSize:12}}>▶️ Play</button>}
