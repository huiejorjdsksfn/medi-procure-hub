/**
 * EL5 MediProcure — send-sms v14.0 PRODUCTION FIX
 * FIXED: serve() → Deno.serve() (Supabase runtime compat)
 * FIXED: status action now calls Twilio API for real account data
 * FIXED: fetch_messages, call, and template actions fully implemented
 * FIXED: Proper field mapping for SMSPage TwilioStatus interface
...
async function loadCreds() {
    ...
    try {
      const { data: rows } = await sb.from("system_settings").select("key,value")
        .in("key", ["twilio_account_sid","twilio_auth_token","twilio_from_number",
          "twilio_messaging_service_sid","twilio_whatsapp_from","at_api_key","at_username"]);
  ...
  // Guard: reject known-bad trial number that does NOT belong to this account
...
function twilioBase(creds: Awaited<ReturnType<typeof loadCreds>>) {
  return {
    url: `https://api.twilio.com/2010-04-01/Accounts/${creds.ACCT}`,
    auth: "Basic " + btoa(`${creds.ACCT}:${creds.AUTH}`),
  };
}
...
async function twilioSend(
  to: string, body: string, ch: "sms" | "whatsapp",
  ...
  try {
    ...
    if (!r.ok) {
      ...
      if (d.code === 21606 || d.code === 21612 || msg.includes("not verified")) {
        return { ok: false, error: `TRIAL_LIMIT: ${msg}. Upgrade account or verify recipient.`, provider: "twilio_trial" };
      }
      return { ok: false, error: `[${d.code}] ${msg}`, provider: "twilio" };
    }
    return { ok: true, sid: d.sid, provider: ch === "whatsapp" ? "twilio_wa" : "twilio_sms" };
  } catch (e: any) {
    return { ok: false, error: `Network error: ${e.message}`, provider: "twilio" };
  }
}
...
async function logSms(to: string, body: string, res: any, meta: any, ch: string, from: string) {
  const entry = {
    ...
    error_msg: res.error || null, provider: res.provider || "twilio",
    module: meta.module || "system", sent_at: new Date().toISOString(),
    recipient_name: meta.name || null, department: meta.dept || null,
  };
  await Promise.allSettled([
    sb.from("sms_log").insert(entry),
    sb.from("reception_messages").insert({
      recipient_phone: to, message_body: body, message_type: ch,
      direction: "outbound", status: res.ok ? "sent" : "failed",
      sent_at: new Date().toISOString(),
    }),
    sb.from("sms_conversations").upsert({
      phone_number: to, last_message: body.slice(0, 100),
  ...
  try {
    ...
    if (action === "status") {
      if (!creds.ACCT || !creds.AUTH) {
        return json({ ok: false, account_status: "credentials_missing",
          friendly_name: null, phone_numbers: [], sms_number: null,
          wa_number: null, msg_svc_sid: null, templates_available: 0,
          api_sid: null, region: "us1", acct_set: false, auth_set: false,
          error: "TWILIO_ACCOUNT_SID or TWILIO_AUTH_TOKEN not set in Supabase secrets",
          hint: "Go to Supabase dashboard → Project Settings → Edge Functions → Secrets" });
      }
      ...
      try {
        ...
        if (numsRes.status === "fulfilled") {
          phoneNumbers = (numsRes.value.incoming_phone_numbers || []).map((p: any) => ({
            number: p.phone_number, friendly: p.friendly_name, sid: p.sid,
      ...
      // Count DB templates
      const { count: tmplCount } = await sb.from("sms_templates")
      ...
      return json({
        ok:                  isOk,
        version:             "14.0",
        account_status:      acctData.status || (isOk ? "active" : "unverified"),
        friendly_name:       acctData.friendly_name || "EL5 MediProcure",
        phone_numbers:       phoneNumbers,
        sms_number:          creds.FROM,
        wa_number:           "whatsapp:" + (creds.FROM_WA.replace("whatsapp:", "")),
        msg_svc_sid:         creds.MSID || null,
        templates_available: (tmplCount || 0) + 19,  // DB + built-in
        api_sid:             creds.ACCT.slice(0, 8) + "…",
        region:              "us1",
        acct_set:            !!creds.ACCT,
        auth_set:            !!creds.AUTH,
        twilio_error:        acctData.code ? `[${acctData.code}] ${acctData.message}` : null,
        wa_join:             WA_JOIN,
        at_set:              !!creds.AT_KEY,
      });
    }
    ...
    if (action === "fetch_messages") {
      ...
      try {
        const { url: base, auth } = twilioBase(creds);
        const r = await fetch(`${base}/Messages.json?PageSize=${b.limit || 50}`, {
        ...
        const messages = (d.messages || []).map((m: any) => ({
          ...
          price: m.price, price_unit: m.price_unit,
        }));
        return json({ ok: true, total: messages.length, messages });
      } catch (e: any) {
        return json({ ok: false, error: e.message, messages: [] });
      }
    }
    ...
    if (action === "call") {
      ...
      try {
        const r = await fetch(
          ...
          { method: "POST",
            headers: { Authorization: "Basic " + btoa(`${creds.ACCT}:${creds.AUTH}`), "Content-Type": "application/x-www-form-urlencoded" },
        ...
        await sb.from("sms_log").insert({
          ...
          message: `[VOICE CALL] ${script.slice(0, 200)}`,
          status: "sent", twilio_sid: d.sid, provider: "twilio_voice",
          module: mod || "system", sent_at: new Date().toISOString(),
        });
        return json({ ok: true, sid: d.sid, status: d.status, to: callTo });
      } catch (e: any) {
        return json({ ok: false, error: e.message, to: callTo });
      }
    }
    ...
    if (action === "save_template") {
      ...
      const { data, error } = await sb.from("sms_templates").insert({
        name: tName, content, category: category || "custom",
    ...
    // ════════════ SEND (default) ════════════
    if (!to || !message) {
      return json({ ok: false, error: "to and message required — or provide an action" }, 400);
    }
    ...
    if (!formattedNums.length) {
      return json({ ok: false, error: "No valid E.164 numbers. Use +2547XX or 07XX format.", tried: rawNums }, 400);
    }
    ...
    return json({
      ...
      formatted_numbers: formattedNums, sms_from: creds.FROM, wa_join: WA_JOIN, results,
    });

  } catch (e: any) {
    console.error("[send-sms v14] Fatal:", e);
