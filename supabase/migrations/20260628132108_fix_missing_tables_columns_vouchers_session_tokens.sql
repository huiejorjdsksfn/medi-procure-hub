-- 1. STORE VOUCHERS TABLE (idempotent)
...
CREATE POLICY "vouchers_select" ON public.vouchers FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "vouchers_insert" ON public.vouchers FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "vouchers_update" ON public.vouchers FOR UPDATE USING (auth.uid() IS NOT NULL);
CREATE POLICY "vouchers_delete" ON public.vouchers FOR DELETE USING (
  EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND role IN ('admin','superadmin','webmaster'))
);

-- 2. QUOTATIONS: add supplier_name
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='quotations' AND column_name='supplier_name' AND table_schema='public') THEN
    ALTER TABLE public.quotations ADD COLUMN supplier_name text;
  END IF;
END $$;

-- 3. CONTRACTS: add contract_type
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='contracts' AND column_name='contract_type' AND table_schema='public') THEN
...
CREATE POLICY "notifications_select_v2" ON public.notifications
  FOR SELECT USING (
    ...
    OR EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND role IN ('admin','superadmin','webmaster'))
  );
...
CREATE POLICY "tokens_select" ON public.user_session_tokens FOR SELECT USING (
  user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.user_roles WHERE user_id=auth.uid() AND role IN ('admin','superadmin','webmaster'))
);
CREATE POLICY "tokens_insert" ON public.user_session_tokens FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "tokens_update" ON public.user_session_tokens FOR UPDATE USING (user_id = auth.uid());
CREATE POLICY "tokens_delete" ON public.user_session_tokens FOR DELETE USING (
  user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.user_roles WHERE user_id=auth.uid() AND role IN ('admin','superadmin','webmaster'))
);
...
BEGIN
  DELETE FROM public.user_session_tokens WHERE expires_at < now() OR is_revoked = true;
END;
$$;
